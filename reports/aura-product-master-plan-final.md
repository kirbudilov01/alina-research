...
Техническая архитектура должна сохранять причинность, контролировать себестоимость и не допустить бесплатное ежедневное видео в MVP.

## Что читатель должен понять

После этой главы читатель должен понимать: Техническая архитектура должна сохранять причинность, контролировать себестоимость и не допустить бесплатное ежедневное видео в MVP.

## Уровни информации в этой главе

| Уровень | Что внутри | Как читать |
| --- | --- | --- |
| Main Narrative | Архитектура, image-first Life Canvas, no free daily video, cost-control. | Читать обязательно. |
| Supporting Evidence | Stack decision, provider logic, unit economics principles, security. | Читать для проверки вывода. |
| Appendix | Provider comparison, full cost tables, database schema, event taxonomy, QA checklist. | Открывать при глубокой проверке или подготовке работы. |
...
В основном narrative фиксируем решение: image-first Life Canvas, controlled AI/image providers, backend state machine, storage, analytics, billing и admin-control. Полная таблица сервисов, цен и вариантов реализации остается в appendix.

Полная доказательная детализация этого блока перенесена в Appendix / Evidence Layer, чтобы основной narrative не превращался в таблицу и сохранял product-first чтение.
## Модель себестоимости: сколько может стоить продукт на разных масштабах

Главный вывод для основной книги: text AI относительно дешевый, image generation становится ключевой переменной статьей, а free daily video нельзя включать в MVP. Полные сценарии на 100 / 1 000 / 10 000 / 100 000 пользователей уходят в appendix.

Полная доказательная детализация этого блока перенесена в Appendix / Evidence Layer, чтобы основной narrative не превращался в таблицу и сохранял product-first чтение.
## Аналитика, платный экран и финмодель: что нужно доказать до ТЗ

В основном narrative оставляем управленческий вывод: считать нужно не абстрактный MAU, а completed loop, returned user, cost per completed loop и paid intent after value. Подробная финмодель и список аналитических событий находятся в appendix.
...
В основной книге достаточно выбора принципа: провайдеры должны быть быстрыми, управляемыми по стоимости, безопасными для персонального контекста и пригодными для image-first MVP. Полное сравнение GPT/Claude/Gemini/DeepSeek/OpenRouter, image и avatar providers переносится в appendix.
...
В main narrative фиксируем не всю сравнительную таблицу, а решение: для MVP нужен управляемый AI brain, image-first avatar/Life Canvas layer, mobile stack, backend, storage, billing и analytics. Полная технологическая матрица остается в appendix.
...
Основной вывод: экономика AURA бьется только при контроле image/video cost, paywall after value и учете cost per completed loop. Детальные сценарии по масштабам переносятся в appendix.
...
Appendix сохраняет детальность и доказательную базу, но не ломает основное product-first чтение.

## Что читатель должен понять

После этой главы читатель должен понимать: Appendix сохраняет детальность и доказательную базу, но не ломает основное product-first чтение.
...
**Почему этот блок здесь.** Здесь важен не сам объем рынка, а вывод для формы продукта: AURA должна занять место между смыслом, действием, памятью и визуальным прогрессом.
## Экономика, провайдеры и финмодель
### Что технически нужно подключать для avatar и Life Series
...
| Какое решение приняли | Сохраняем этот блок как часть единой логики продукта, но не позволяем ему размыть главный loop. |
| Как это влияет на продукт | Следующий слой работы должен усиливать Episode -> Action -> Reset -> Reflection -> Life Canvas -> Tomorrow Hook. |
## Расширенная детализация решения
