...
## Startup Whitepaper + Evidence Report

AURA - это потребительский AI-продукт на пересечении self-care, личного смысла, маленьких действий, визуального прогресса и возвращаемости. Главная ставка продукта: пользователь проходит короткую ежедневную петлю и видит, что его Life Canvas изменился не случайно, а как след собственного действия.
...
| Шаг | Элемент | Роль | Что ломается без него |
| --- | --- | --- | --- |
| 01 | Episode | Смысл дня | Пользователь не чувствует персональности. |
| 02 | Action | Маленький шаг | Инсайт остается пассивным контентом. |
| 03 | Reset | Снятие сопротивления | Переход к действию становится слишком тяжелым. |
| 04 | Reflection | Доказательство опыта | Нет связки между действием и памятью. |
| 05 | Life Canvas | Видимый след | Образ воспринимается как случайная AI-картинка. |
| 06 | Tomorrow Hook | Возврат | Нет причины вернуться завтра. |

## Executive summary

| Вопрос | Ответ |
| --- | --- |
| Почему рынок существует | AURA опирается не на один рынок, а на пересечение mindfulness/reset, astrology/self-discovery, coaching/habits, avatar/identity и gaming/progression. Каждый рынок уже содержит платное поведение и привычные сценарии. |
| Почему есть деньги | Соседние категории монетизируют подписки, premium-контент, персональные интерпретации, avatars, progression и digital identity. Для AURA это не гарантия выручки, но сильный направленный сигнал. |
| Почему есть аудитория | Первые сегменты уже используют ритуалы саморефлексии, привычки, reset-практики, avatars/future-self и персональный контент. Это не холодный рынок. |
| Где белое пятно | Белое пятно не в отсутствии конкурентов. Оно в разорванной причинной петле: смысл без действия, действие без образа, avatar без причины, progress без эмоционального контекста. |
| Почему такой MVP | Первый продукт должен доказать одну цепочку: Episode -> Action -> Reset -> Reflection -> Life Canvas -> Tomorrow Hook. Все остальное вторично. |
| Почему такой GTM | Первые пользователи нужны не как трафик, а как доказательство поведения. Поэтому старт идет через interviews, concierge cohort, прототип и только затем публичные каналы. |
| Почему такой стек | Стек должен сохранять причинность, контролировать себестоимость и давать аналитику loop. Поэтому image-first, no free daily video, backend state machine, cost logs, analytics и admin-control. |
<!-- PAGEBREAK -->
# ЧАСТЬ 1. Рынок, деньги и белое пятно

## Executive summary раздела

| Логика | Смысл |
| --- | --- |
| Что доказываем | Есть ли вокруг AURA реальный мировой спрос, платное поведение и незанятая форма продукта? |
| Почему это важно | Без этого AURA останется красивой идеей без доказательной базы. |
| Какой вывод нужен | Нужно понять, какие категории дают продуктовые слои, а не просто собрать большой список конкурентов. |
...
| Итог | Ответ |
| --- | --- |
| Что узнали | Рынок вокруг AURA существует как совокупность пяти соседних категорий, а не как уже готовая отдельная категория. |
| Какое решение приняли | Позиционировать AURA как causal life-series/self-change product, а не как horoscope app, avatar app или habit tracker. |
| Что это меняет в продукте | MVP должен проверять белое пятно через причинную петлю, а не через количество функций. |
<!-- PAGEBREAK -->
# ЧАСТЬ 2. Продукт и MVP

## Executive summary раздела

| Логика | Смысл |
| --- | --- |
| Что доказываем | Что именно строим и как пользователь должен пройти первый причинный опыт? |
| Почему это важно | Самая большая недостающая информация после market research - не еще один рынок, а конкретный пользовательский путь. |
| Какой вывод нужен | MVP должен быть не маленькой версией большого приложения, а точным тестом центральной петли. |
...
| Итог | Ответ |
| --- | --- |
| Что узнали | AURA становится продуктом, когда пользователь может объяснить, почему изменился Life Canvas. |
| Какое решение приняли | Первый релиз строится вокруг Episode, Action, Reset, Reflection, Life Canvas и Tomorrow Hook. |
| Что это меняет в продукте | Видео, маркетплейс, сообщество и социальная сеть исключаются из первого продукта, пока не доказана базовая петля. |
<!-- PAGEBREAK -->
# ЧАСТЬ 3. Технологии, архитектура и экономика

## Executive summary раздела

| Логика | Смысл |
| --- | --- |
| Что доказываем | Можно ли это собрать технически и не сломать себестоимость? |
| Почему это важно | AURA легко превратить в дорогую AI-игрушку. Архитектура должна защищать продукт от этой ошибки. |
| Какой вывод нужен | Выбрать стек, который быстро проверяет петлю, хранит память, контролирует cost и не обещает ежедневное бесплатное видео. |
...
| Итог | Ответ |
| --- | --- |
| Что узнали | Технически продукт реалистичен, если держать MVP image-first и считать стоимость каждого completed loop. |
| Какое решение приняли | В первом продукте нужны mobile app, backend state machine, Postgres, storage, AI/image layer, billing, analytics и admin. |
| Что это меняет в продукте | Самое важное ограничение: no free daily video. Видео можно тестировать позже как premium/token-момент. |
<!-- PAGEBREAK -->
# ЧАСТЬ 4. Монетизация и выход на рынок

## Executive summary раздела

| Логика | Смысл |
| --- | --- |
| Что доказываем | Почему люди вернутся, почему заплатят и как получить первых пользователей? |
| Почему это важно | Даже сильный продукт провалится, если команда начнет с рекламы вместо проверки поведения. |
| Какой вывод нужен | Запуск должен искать не просмотры, а доказательства: loop completion, causality comprehension, retention и paid intent. |
...
| Итог | Ответ |
| --- | --- |
| Что узнали | Первые 100 пользователей не покупаются рекламой: они нужны как проверка языка, доверия, причинности и повторного использования. |
| Какое решение приняли | GTM начинается с warm contacts, interviews, concierge cohort и content tests; публичное масштабирование идет позже. |
| Что это меняет в продукте | Маркетинг должен продавать не AI-картинку, а понятную трансформацию: действие сегодня меняет Life Canvas завтра. |
<!-- PAGEBREAK -->
# ЧАСТЬ 5. План сборки и решение о запуске

## Executive summary раздела

| Логика | Смысл |
| --- | --- |
| Что доказываем | Как превратить research в работу команды, бюджет и go/no-go решение? |
| Почему это важно | После доказательной базы нельзя уходить в бесконечное документирование. Нужно перейти к Figma, прототипу, интервью и первой когорте. |
| Какой вывод нужен | Команда должна строить именно проверку loop, а не весь возможный продукт. |
...
| Итог | Ответ |
| --- | --- |
| Что узнали | Research уже достаточно силен, чтобы перестать спорить о рынке и начать спорить о прототипе. |
| Какое решение приняли | Следующий шаг: Figma wireframes, low-fi prototype, 20 интервью и 30 concierge users. |
| Что это меняет в продукте | Главный риск проекта теперь внутри прототипа: сможет ли пользователь объяснить изменение Life Canvas как след собственного действия. |
<!-- PAGEBREAK -->
# Финальное решение

| Вопрос | Решение |
| --- | --- |
| Что строим | Мобильное приложение AURA: персональный daily life-series loop, где пользователь получает Episode, делает маленький Action, проходит Reset, фиксирует Reflection и видит причинное изменение Life Canvas. |
| Для кого | Первые сегменты: spiritual self-improvers, habit/progress users, reset users и avatar/future-self users. Приоритет - люди, у которых уже есть недавнее поведение вокруг self-work. |
| Почему может выстрелить | AURA соединяет существующие платные спросы в новый causal loop: смысл + действие + память + визуальный след + возврат. |
| Почему может провалиться | Если Life Canvas будет восприниматься как случайная AI-картинка, если действие не будет выполняться, если себестоимость image/video станет выше монетизации или если onboarding вызовет недоверие. |
| Что должно быть в MVP | Onboarding, consent, profile, season, daily episode, daily action, reset, reflection, Life Canvas, tomorrow hook, basic memory, analytics, paywall after value. |
| Что запрещено в MVP | Free daily video, marketplace, community, social network, AR, complex coaching platform, heavy avatar studio, endless content library. |
| Метрики успеха | Activation, completed loop, causality comprehension, D1/D7 retention, paid intent, cost per completed loop. |
| Метрики остановки | Пользователь не понимает causal link, не делает действие, не возвращается на Day 2, не доверяет персональным данным или не видит ценности в paywall. |
