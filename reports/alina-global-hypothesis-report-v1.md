...
| Сервис / источник | Как может использоваться в АУРЕ | Как читать для решения | Источник |
| --- | --- | --- | --- |
| OpenAI Image Generation API | Генерация и редактирование image/avatar cards, визуальных карточек эпизода, future-self образов. | Подходит для image-first MVP; цену считать по качеству, размеру и количеству генераций. | https://openai.com/index/image-generation-api/ |
| Replicate | Доступ к разным image/video/open-source моделям через API и pay-as-you-go эксперименты. | Удобно для прототипирования и сравнения моделей без жесткой привязки к одному поставщику. | https://replicate.com/pricing |
| HeyGen API | Avatar video, talking avatar, digital twin / instant avatar сценарии. | Смотреть как premium-видеослой, не как ежедневную бесплатную механику. | https://developers.heygen.com/docs/pricing |
| D-ID API | Talking avatars / agents / video presenter сценарии. | Нужна отдельная проверка pricing, consent, качества и ограничений по образу. | https://help.d-id.com/hc/en-us/articles/31262544439697-Is-there-an-API-that-I-can-use-to-create-Agents |
| Tavus API | Conversational video interface и AI-replica сценарии. | Больше подходит для дорогого human-like слоя или future-self conversation, чем для простого MVP. | https://docs.tavus.io/api-reference |
| Runway API | Кинематографичная video generation, трейлеры сезонов, visual episodes. | Считать по секундам/credits; использовать для редких вау-моментов. | https://docs.dev.runwayml.com/guides/pricing |
| ElevenLabs API | Text-to-speech, voice narration, эмоциональный voice reset. | Добавлять после проверки текстовой петли; голос повышает стоимость и ожидание качества. | https://elevenlabs.io/docs/overview/intro |
| Supabase | Auth, Postgres, storage, edge functions, база эпизодов и состояния avatar. | Подходит как быстрый backend для MVP, но нужно считать MAU/storage/egress. | https://supabase.com/pricing |

Технический вывод: реализуемость высокая, если не начинать с полностью видеоцентричного продукта. Самый здоровый порядок такой: сначала mobile/web MVP с текстом, действиями, reset и слоистым avatar; затем image/avatar generation по лимитам; затем voice; затем редкий premium video-avatar. Так продукт остается проверяемым, экономика - управляемой, а центральная идея “сериал о себе” не зависит от самого дорогого провайдера.
