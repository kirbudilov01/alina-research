...
| День 7 | Финал первого сезона: 7 эпизодов, карта действий, before/after Life Canvas, summary черт героя. | “Я вижу, что неделя была не случайной, у нее появилась история”. | Нужен season recap и первый сильный платный/возврат момент. | Season completion, save/share recap, пробный период/subscription intent. |
...
| Видимая эволюция avatar | После действия меняется Life Canvas: свет, черта, предмет, фон, карта сезона. | Avatar viewed, saved, shared, paid visual intent. | Нужна понятная причинность “я сделал -> герой изменился”. |
| Маленькое действие | Пользователь не просто читает интерпретацию, а выбирает один шаг по силам. | Action chosen, действие completed, reflection saved. | Три уровня сложности: мягкий, обычный, смелый. |
| Reset перед действием | Короткий ritual/голос/breathing переводит смысл в состояние готовности. | Reset start/completion, действие completion after reset. | Reset должен быть коротким и связанным с эпизодом. |
| Weekly recap | Неделя собирается в понятную карту изменений и дает “я не стоял на месте”. | Season completion, recap save/share, subscription intent. | День 7 должен быть отдельным сильным моментом продукта. |
| Мягкий прогресс без стыда | Продукт возвращает через любопытство и заботу, а не через наказание за пропуск. | Reactivation after missed day, notification opt-in. | Нужны comeback-эпизоды и отсутствие агрессивного streak. |
...
| --- | --- | --- | --- |
| Неделя 1 | 20-30 интервью с первичными сегментами, ручной ручную проверку 10-15 ближайших конкурентов, сбор платный экран/feature экраны. | Уточненный ICP, язык боли, карта “что уже есть / чего нет”, первые формулировки офферов. | Пользователи описывают недавнее поведение и понимают идею без долгого объяснения. |
