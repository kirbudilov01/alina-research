...
| Семейство источников | Исходных строк | После уникализации | Ниш | Как использовать |
| --- | ---: | ---: | --- | --- |
| Steam: PC/gaming продукты как бенчмарк progression и возврат механик | 27642 | 17505 | 5 | используем как бенчмарк механик и saturation, не как прямой TAM АУРЫ |
| Desktop/Mac/PC stores: соседние desktop-продукты и productivity/wellness контекст | 15397 | 2478 | 5 | поддерживающий слой для проверки позиционирования, pricing или соседних продуктов |
| App Store: карточки приложений, категории, рейтинги, отзывы и ссылки | 12000 | 6958 | 5 | ближе всего к конкурентной карте consumer-приложений |
| itch.io: indie/game эксперименты, avatar/identity и cozy/progression паттерны | 7047 | 5696 | 3 | используем как бенчмарк механик и saturation, не как прямой TAM АУРЫ |
| Google Play / Android: кросс-проверка карточек и pricing-сигналов | 2527 | 1646 | 5 | ближе всего к конкурентной карте consumer-приложений |
| Reddit/forum/community: VOC, боли, обходные решения и язык пользователей | 2339 | 1852 | 5 | используем как язык боли/VOC, не как репрезентативный опрос |
| Сайты компаний/pricing pages: позиционирование, paywall и paid-depth сигналы | 560 | 482 | 5 | поддерживающий слой для проверки позиционирования, pricing или соседних продуктов |
| Chrome Web Store: browser-extension слой и lightweight utility patterns | 252 | 252 | 5 | поддерживающий слой для проверки позиционирования, pricing или соседних продуктов |
| прочие нормализованные source rows | 251 | 251 | 5 | поддерживающий слой для проверки позиционирования, pricing или соседних продуктов |

Трассировка источников хранится в локальных файлах: `data_processed/cross_source_universe_dedup.csv`, `data_processed/competitor_feature_matrix.csv`, `data_processed/top100_competitor_review_scorecard.csv`, `data_processed/review_signal_matrix.csv`, `data_processed/russian_source_provenance_index.csv` и `data_processed/market_source_registry.csv`. Важно читать это аккуратно: source rows показывают масштаб и происхождение данных, но финальные продуктовые выводы нельзя усиливать без ручного walkthrough, интервью и прототипных сессий.
...
| coaching/self-improvement | прямой соседний рынок | $300M | $210M | широкий диапазон источников, нужен консервативный диапазон | использовать как направленный рыночный ориентир до ручного paywall, ICP и доказательства готовности платить |
...
| --- | --- | --- | --- |
| Базовый рынок | Мобильные consumer-приложения | Мы делаем мобилку, поэтому конкурируем за привычку открыть приложение, пройти короткую сессию и вернуться завтра. | Считать верхним уровнем, но не использовать как доказательство спроса само по себе. |
...
| --- | ---: | --- | --- |
| Mindfulness / reset | 3.82% | возможность есть, но нужна выборочная ручная проверка | H3 можно держать как узкое направленное белое пятно: кандидаты с похожей полной петлей редки, но выборочная проверка обязательна. |
| Avatar / identity | 2.83% | возможность есть, но нужна выборочная ручная проверка | H3 можно держать как узкое направленное белое пятно: кандидаты с похожей полной петлей редки, но выборочная проверка обязательна. |
...
| 1. Рыночный прототип | В каких рынках эта идея вообще живет и где есть деньги? | Карта пяти категорий: wellness, self-improvement, astrology, avatar/identity, gaming/progression. | Если деньги видны только в нерелевантных рынках, H2 ослабляется. |
...
| 3. Конкурентный прототип | Есть ли у конкурентов такая же связка внутри onboarding? | Ручной проход по P0-конкурентам: смысл, действие, reset, avatar, paywall. | Если найден скрытый клон, нужно сузить или изменить ставку. |
...
| 6. Экономический прототип | При каком прайсинге, частоте генераций и конверсии модель имеет смысл? | Unit-модель: подписка, лимиты генераций, себестоимость, gross margin, маркетинг. | Если себестоимость съедает маржу, меняем механику или монетизацию до разработки. |
...
| 5. Voice / narration | Озвучить эпизод, reset или обращение future-self. | TTS API, например ElevenLabs или аналог; аудио-кэширование. | Не обязательно, но может усилить эмоциональность. | Голос повышает стоимость и требования к качеству; плохой voice ломает ощущение премиальности. |
| 6. Video-avatar / living avatar | Сделать ролики “лучшая версия себя”, трейлер сезона, special episode или talking avatar. | HeyGen / D-ID / Tavus для avatar-video; Runway или похожие video generation API для cinematic-сцен. | Не ставить в ежедневную бесплатную норму; тестировать как premium. | Самый дорогой слой: себестоимость, latency, retries, права на лицо/образ и consent. |
| 7. Память сериала и аналитика | Хранить эпизоды, действия, состояния avatar, возврат, paywall, понимание причинности. | Postgres/Supabase, object storage, event analytics, A/B flags. | Обязательно с первой версии. | Без аналитики невозможно понять, где ломается петля: смысл, действие, reset или avatar. |
...
| Avatar video API | D-ID API | Talking avatars / agents / video presenter сценарии. | Планы от trial до paid; trial дает ограниченные минуты video/streaming, далее кредиты/планы. | Проверять для talking portrait и agent-слоя; важно смотреть watermark, лицензию и права на персональный avatar. | https://www.d-id.com/pricing/api/ |
| Conversational avatar API | Tavus API | Conversational video interface и AI-replica сценарии. | API/pricing требует отдельной коммерческой проверки по выбранному сценарию. | Больше подходит для дорогого future-self conversation, чем для простого MVP. | https://docs.tavus.io/api-reference |
...
| Avatar video platform/API | Colossyan | AI avatars, custom avatars, AI voices, auto-translation, API для автоматизации видео. | Starter около $27/month с 15 min/month; Business около $88/month, API included/add-on по плану. | Полезен как бенчмарк “аватар + обучение + сценарии”, но для АУРЫ может быть слишком корпоративным. | https://www.colossyan.com/pricing |
...
| Voice API | ElevenLabs API | Text-to-speech, voice narration, эмоциональный voice reset. | Считать по плану, символам/минутам и требованиям к voice clone. | Добавлять после проверки текстовой петли; голос повышает стоимость и ожидание качества. | https://elevenlabs.io/docs/overview/intro |
...
| Backend | Firebase | Auth, analytics, remote config, push notifications, mobile backend. | Spark/Blaze; usage-based pricing для Firebase/Google Cloud ресурсов. | Альтернатива Supabase, особенно если делать mobile-first и быстро включать аналитику/remote config. | https://firebase.google.com/pricing |
| Payments | RevenueCat | Подписки, paywalls, entitlement logic, App Store/Google Play billing, A/B paywall tests. | Считать по revenue/MAU и выбранному плану. | Практически обязательный кандидат для мобильной подписочной модели, чтобы не писать billing-логику с нуля. | https://www.revenuecat.com/pricing/ |
| Analytics | PostHog | Product analytics, funnels, feature flags, session replay, experiments. | Usage-based по events/session replay/features. | Нужен для проверки, где ломается петля: onboarding, первый avatar, действие, reset, paywall. | https://posthog.com/pricing |

Отдельно нужно рассмотреть локальный/open-source слой. Он не заменяет hosted API в первом прототипе, потому что требует GPU, MLOps, очередей, мониторинга качества, прав на модели и инженеров, которые будут чинить пайплайн. Но он важен стратегически: если у АУРЫ выстрелит регулярная визуальная петля, локальные модели могут снизить переменную себестоимость, дать больше контроля над стилем и убрать зависимость от одного avatar-провайдера.
...
| MuseTalk | Real-time/high-quality lip synchronization через latent-space inpainting. | Кандидат для более качественного talking-face слоя после базовой проверки работоспособности. | GPU-инференс и интеграция; нужно тестировать latency и стабильность на пользовательских лицах. | Research/open-source слой: до production нужен отдельный техаудит качества, лицензий и воспроизводимости. | https://github.com/TMElyralab/MuseTalk |
...
| --- | --- | --- | --- |
| MVP 0: доказать петлю | Backend, LLM-сценарии, static/layered avatar, analytics, paywall foundation. | Supabase/Firebase, OpenAI Image/Stability/Replicate, RevenueCat, PostHog. | Проверяем: человек понимает “сериал о себе”, возвращается и завершает действия. |
| MVP 1: усилить образ | Периодические future-self cards, сезонные изменения, avatar progress, voice reset. | Image API + ElevenLabs/Cartesia + storage/CDN. | Проверяем: визуальный образ повышает возврат и willingness to pay. |
...
| --- | --- | --- | --- |
| Onboarding | app_open, onboarding_start, birth_data_added, theme_selected, consent_accepted, onboarding_completed | Понять, не пугает ли вход через дату рождения, символы и персональные данные. | Если drop-off высокий до первого эпизода, упрощаем вход и переносим часть вопросов позже. |
| Первый момент пользы | first_episode_generated, first_avatar_seen, first_reset_started, first_step_selected, first_step_completed | Проверить, случается ли “ага, это про меня” до paywall. | Paywall нельзя ставить до того, как пользователь увидел личный смысл и действие. |
| Ежедневная петля | daily_episode_opened, reset_completed, reflection_saved, streak_updated, avatar_change_seen | Понять, есть ли ритуал и что именно возвращает: текст, reset, avatar, прогресс или напоминание. | Сохраняем только те механики, которые реально двигают возврат на D1/D7. |
| Avatar и visual value | avatar_card_generated, avatar_card_saved, avatar_shared, video_avatar_requested, video_avatar_completed | Отделить “вау” от платной ценности: человек просто смотрит или готов платить/делиться. | Если video смотрят, но не покупают, оставляем его как маркетинговый hook, а не базовую механику. |
| Paywall и деньги | paywall_viewed, trial_started, purchase_started, purchase_completed, purchase_failed, subscription_cancelled | Проверить цену, timing, trial, годовой план и формулировку платной глубины. | Если конверсия слабая, меняем не только цену, но и момент paywall и состав платной глубины. |
| Качество и безопасность | content_regenerated, content_disliked, safety_flagged, support_contacted, refund_requested | В персональном/spiritual продукте доверие важнее количества генераций. | Если много dislike/refund, усиливаем ограничения, объяснение и контроль пользователя. |
...
| --- | --- | --- | --- |
| App Store / Google Play commission | Уменьшает gross revenue до net revenue; для подписок и small-business сценариев часто считать 15%, но условия нужно подтвердить по аккаунту и стране. | Apple Small Business Program; Google Play service fees | В базовой модели считать 15% как optimistic/eligible и 30% как stress case. |
| RevenueCat / subscription infrastructure | Нужен для entitlement, paywall, trials, experiments, восстановлений покупок и связки с аналитикой. | RevenueCat pricing | В MVP экономит разработку; в финмодели закладывать fee/процент после бесплатного порога. |
| Analytics / experiments | События, funnels, feature flags, session replay, A/B paywall. | PostHog pricing | Это не “nice to have”: без аналитики нельзя считать возврат, конверсию и payback. |
| AI text cost | Стоимость генерации эпизода, интерпретации, reset, micro-coaching. | Выбранный LLM provider | Должна быть низкой и входить в ежедневную подписочную петлю. |
| Image/avatar cost | Стоимость карточек, future-self образов, сезонных изменений. | OpenAI/Stability/Replicate или локальный visual stack | Давать лимиты: например, daily text бесплатно/в подписке, image - несколько раз в неделю или по плану. |
| Video/avatar cost | Стоимость секунд или минут talking/cinematic avatar. | HeyGen/D-ID/AKOOL/Creatify/Runway/Luma/Tavus | Не включать безлимитно в подписку; продавать как premium, token или milestone reward. |
| Refunds, support, retries | Ошибки генерации, неудачные образы, жалобы на подписку, повторные попытки. | Внутренняя аналитика и support tags | Добавлять buffer к себестоимости, особенно в video/avatar сценариях. |
...
- Net revenue per paying user = цена подписки или ARPPU минус store fee, refunds и subscription tooling.
- Product gross margin = net revenue минус AI text, image/video/avatar generation, voice, storage, analytics, support и retries.
- Contribution margin = product gross margin минус paid acquisition cost.
- Payback = CAC / monthly contribution margin.
- Допустимый CAC = LTV * целевая доля на маркетинг, где LTV зависит от повторных возвращений, churn и annual/monthly mix.

| Сценарий | Что проверяем в цене | Себестоимость | Монетизация | Вывод |
| --- | --- | --- | --- | --- |
| Базовая подписка: текст + reset + память | $7.99-9.99/month или $39.99-59.99/year как проверяемый диапазон, не финальная цена. | Низкая: LLM, storage, analytics, subscription tooling. | Подписка после первого момента ценности: бесплатный daily teaser и платная глубина. | Это самый здоровый MVP-сценарий: высокий шанс сохранить маржу и проверить повторный возврат. |
| Подписка с image/avatar | Подписка выше или лимиты: N image/avatar moments в неделю/месяц. | Средняя: image API или локальная генерация, moderation, retries. | Платная глубина: future-self cards, сезонные изменения, visual archive. | Работает, если avatar повышает возврат на D7/D30 или конверсию в оплату. |
| Premium video-avatar | Токены, разовые покупки, special episodes или дорогой premium tier. | Высокая: секунды/минуты видео, voice, retries, ожидание качества. | Не безлимит; milestone reward + paid packs. | Делать только после проверки готовности платить, иначе можно получить красивый продукт с плохой маржей. |
| Гибридная модель на масштабе | Подписка за ежедневную петлю + токены за video + annual plan для cashflow. | Управляемая: hosted API в начале, локальный stack на масштабе. | Лучший кандидат после MVP: подписка удерживает, premium увеличивает ARPPU. | Целевое направление, если базовая петля доказала повторный возврат. |

Четвертая часть - маркетинг и payback. В голосовой логике это следующий слой после продуктовой экономики: сначала считаем продуктовую маржу без маркетинга, потом проверяем, выдерживает ли она привлечение. Для АУРЫ это особенно важно, потому что avatar/future-self может хорошо работать в креативах, но дорогой AI-видеослой может съесть деньги быстрее, чем платный пользователь окупится.

| Канал | Что тестируем | Метрика | Риск |
| --- | --- | --- | --- |
| Organic / TikTok / Reels | “Мой сериал о себе”, future-self, before/after avatar, архетип дня, визуальная трансформация. | Share rate, install CVR, activation to first episode, возврат на D1. | Может привести любопытных пользователей, которые смотрят вау, но не платят. |
| Influencers / spiritual creators | Аудитория уже верит в персональные интерпретации и практики. | CAC by creator, trial start rate, paid conversion, refund rate. | Важно не уйти в обещания “судьбы” и не потерять доверие/safety. |
| App Store Optimization | Mindfulness, horoscope, manifestation, avatar, self-care keywords могут давать intent. | Keyword rank, product page CVR, trial start, paid conversion. | Слишком широкая категория даст дорогой/размытый трафик. |
| Paid social | Тестировать только после понятной unit-модели и paywall. | CAC, trial-to-paid, D7/D30, payback period. | Если CAC выше допустимого, нужно менять pricing, annual mix или бесплатные лимиты. |
...
Для этой версии исследования собрана отдельная Excel-модель: `output/finance/AURA_financial_model_v1.xlsx`. Это не финальный forecast, а первая управленческая экономика, которая показывает, какие допущения делают продукт жизнеспособным. Главный результат модели: консервативный сценарий не сходится, если платящих пользователей мало; базовый и оптимистичный сценарии начинают выглядеть рабочими только при строгом ограничении AI-себестоимости, хорошем trial-to-paid и вынесении video-avatar в premium/token слой.

| Сценарий | Масштаб | Net revenue | Маржа | Payback | Вывод |
| --- | --- | --- | --- | --- | --- |
| Conservative | 10,000 MAU / 84 paid subscribers | $413 net revenue / month | 29.2% product gross margin | 23.2 months | Не проходит: мало платящих пользователей, CAC и AI-cost слишком тяжелые для ранней базы. |
| Base | 50,000 MAU / 1,650 paid subscribers | $13.1k net revenue / month | 68.8% product gross margin | 5.5 months | Проходит как MVP-экономика, если video ограничен, images лимитированы, а ежедневная петля остается дешевой. |
| Upside | 150,000 MAU / 8,370 paid subscribers | $97.0k net revenue / month | 70.6% product gross margin | 4.8 months | Выглядит как сильный scale-сценарий, но требует повторного возврата, annual mix и работающей premium-token логики. |
...
| --- | --- | --- |
| Сделать daily-петля дешевым | Текст, reset, память и static/layered avatar должны быть основной ежедневной нормой. | Именно это сохраняет product gross margin. |
| Лимитировать image/avatar generation | Давать визуальные моменты по плану, milestone или платную глубину, а не безлимитно. | Image-cost быстро растет на активной базе, даже если unit cost кажется маленьким. |
| Video-avatar только premium | HeyGen/D-ID/AKOOL/Runway/Luma использовать для special episodes, token packs или сезонных трейлеров. | Видео дает вау, но разрушает маржу при ежедневном бесплатном использовании. |
| Поднимать annual mix | Продавать годовой план после первого сильного момента ценности и нескольких завершенных эпизодов. | Annual улучшает cashflow и позволяет выдержать CAC. |
| Считать CAC через payback | Paid social запускать только после видимой trial-to-paid и D7/D30 возврат. | Даже при хорошей product gross margin маркетинг может сделать первый месяц отрицательным. |
...
### Самопроверка исследования перед передачей

Я отдельно перепроверяю логику отчета как продуктовый редактор, а не как генератор текста. Главный вопрос: ведет ли документ читателя по той цепочке, которую мы хотим показать клиенту, или просто складывает много данных рядом. Текущая версия уже стала последовательной: идея -> пять направлений -> масштаб данных -> деньги -> конкуренты -> повторяющиеся паттерны -> белое пятно -> аудитория -> продуктовая петля -> техническая реализуемость -> монетизация -> финмодель -> следующий план проверки. Но у исследования честно остаются зоны, где нужна следующая ручная работа.

| Блок | Оценка | Что уже держится | Что еще усилить |
| --- | --- | --- | --- |
| Логика повествования | Стало заметно лучше | Гипотезы идут последовательно, и каждая следующая часть отвечает на предыдущий риск. | Нужно в следующей версии еще сильнее сшить первые страницы с финальной рекомендацией, чтобы executive reader быстрее видел “почему АУРА”. |
| Масштаб данных | Сильный слой | Есть счетчики по пяти нишам, общая база, источники, конкуренты, анализ/VOC и pricing/paywall слой. | Не все 37k+ уникальных объектов вручную проверены; это честно обозначено как discovery coverage, а не финальная ручная экспертиза. |
| Конкурентные паттерны | Рабочий слой | Видны повторяющиеся функции: daily-петля, reset, streak/progress, avatar/identity, AI personalization, community, платная глубина. | Нужен отдельный ручную проверку топ-конкурентов с экранами onboarding/paywall, чтобы доказать, что полной петли у них нет. |
| Продуктовое ядро | Сформулировано | Ядро звучит как “личный смысл -> маленькое действие -> reset -> avatar/progress -> возврат завтра”, а не просто “астро + аватар”. | Нужно проверить на людях, считывают ли они причинность avatar и не кажется ли это декоративным. |
| Техническая часть | Сильно расширена | Есть hosted API, avatar/video/voice сервисы, локальные модели, этапность MVP и scale-маршрут. | Нужен отдельный mini-RFP/таблица выбора поставщиков после того, как будет выбран точный video/avatar сценарий. |
| Монетизация и финмодель | Добавлена как отдельный слой | Есть события аналитики, paywall-логика, статьи себестоимости, формулы и сценарии подписка/image/video/hybrid. | Нужна отдельная таблица финмодели с консервативным, базовым и оптимистичным сценариями после проверки цен, повторного возврата и готовности платить. |
| Человечность текста | Лучше, но требует финальной редакторской вычитки перед клиентом | Текст на русском, с последовательным объяснением “мы думаем -> проверили -> вывод”. | После следующего пакета данных стоит пройти документ еще раз не на факты, а на тон: убрать сухие англицизмы и повторяющиеся формулировки. |

Самая честная оценка текущего состояния: отчет уже можно читать как полноценную исследовательскую рамку, но он еще не является финальным инвестиционным memo или готовым ТЗ. Его сила - в широкой карте рынка, логике гипотез и сформулированном продуктово-техническом пути. Его слабое место - в том, что ручная проверка экранов, paywall и пользовательских реакций еще должна догнать масштаб desk research. Поэтому следующий шаг должен быть не “еще больше данных вообще”, а точечное усиление: ручную проверку конкурентов, spreadsheet финмодели и 8-12 прототипных интервью.
