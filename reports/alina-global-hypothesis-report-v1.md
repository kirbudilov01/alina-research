...
Эта логика важна для продукта: одна и та же MVP-петля должен быть проверен разными входами. Если spiritual self-improver видит в продукте “очередное гадание”, гипотезы 1 и 6 слабеют. Если habit/progress user видит “еще один task manager”, гипотеза 4 слабеет. Если reset user не связывает calm-down с действием, петля распадается. Поэтому сегментация сразу переводится в проверочные вопросы, а не остается маркетинговой типологией.
...
| --- | ---: | --- | --- |
| Mindfulness / reset | 3.82% | возможность есть, но нужна выборочная ручная проверка | гипотеза 3 можно держать как узкое направленное белое пятно: кандидаты с похожей полной петлей редки, но выборочная проверка обязательна. |
| Avatar / identity | 2.83% | возможность есть, но нужна выборочная ручная проверка | гипотеза 3 можно держать как узкое направленное белое пятно: кандидаты с похожей полной петлей редки, но выборочная проверка обязательна. |
| Astrology / esoterics | 13.70% | рынок видим, но вывод о белом пятне слабый без новых доказательств | гипотеза 3 не усиливать: плотность/контекст/прямота пока слишком неоднозначны. |
| Coaching / self-improvement | 13.02% | рынок видим, но вывод о белом пятне слабый без новых доказательств | гипотеза 3 не усиливать: плотность/контекст/прямота пока слишком неоднозначны. |
...
### Итог по белом пятне и аудитории
...
| Voice/STT API | Cartesia | TTS/STT и голосовые agent-сценарии, если АУРА пойдет в аудио/разговорный режим. | Кредитная модель; STT указан как credits per second, голос-agent минуты считаются отдельно. | Резервный вариант для голос-first механик и быстрых low-задержку сценариев. | https://cartesia.ai/pricing |
...
| Payments | RevenueCat | Подписки, платные экраны, логика доступа, App Store/Google Play billing, A/B-тесты платного экрана. | Считать по revenue/MAU и выбранному плану. | Практически обязательный кандидат для мобильной подписочной модели, чтобы не писать billing-логику с нуля. | https://www.revenuecat.com/pricing/ |
| Analytics | PostHog | Product analytics, воронки, флаги функций, записи сессий, эксперименты. | по фактическому использованию по событиям, записям сессий и функциям. | Нужен для проверки, где ломается петля: первый пользовательский опыт, первый avatar, действие, reset, платный экран. | https://posthog.com/pricing |
...
| Первый момент пользы | первый эпизод создан, первый avatar показан, reset запущен, первый шаг выбран, первый шаг завершен | Проверить, случается ли “ага, это про меня” до платного экрана. | Paywall нельзя ставить до того, как пользователь увидел личный смысл и действие. |
...
### Что еще нужно усилить до версии 10/10

Текущая версия уже ведет читателя по правильной цепочке: идея -> пять направлений -> масштаб данных -> деньги -> конкуренты -> повторяющиеся паттерны -> белое пятно -> аудитория -> продуктовая петля -> техническая реализуемость -> монетизация -> финмодель -> план проверки. Но чтобы отчет воспринимался как дорогая полноценная работа, а не как черновик большого анализа, нужно усилить несколько мест.

| Блок | Оценка | Что уже держится | Что еще усилить |
| --- | --- | --- | --- |
| Логика повествования | Сильная основа | Гипотезы идут последовательно, и каждая следующая часть отвечает на предыдущий риск. | На первых страницах нужно еще быстрее показать финальную рекомендацию: почему именно АУРА, почему сейчас и почему начинать с этой петли. |
| Масштаб данных | Сильный слой | Есть счетчики по пяти нишам, большая база, источники, конкуренты, пользовательские сигналы и слой цен/платных экранов. | Нужно сделать отдельную ручную выборку главных конкурентов, чтобы читатель видел не только масштаб базы, но и качество первичного опыта внутри приложений. |
| Конкурентные паттерны | Рабочий слой | Видны повторяющиеся функции: ежедневная петля, reset, streak/progress, avatar/identity, AI-персонализация, community, платная глубина. | Нужен отдельный ручной разбор 15-25 главных конкурентов с экранами первого опыта и платного экрана, чтобы доказать, что полной петли у них нет. |
...
| Техническая часть | Сильно расширена | Есть карта сервисов для текста, изображений, голоса, video/avatar, платежей, аналитики и локальных моделей. | После выбора точного avatar-сценария нужна короткая таблица выбора поставщиков: качество, цена, права, скорость, риски и роль в MVP. |
| Монетизация и финмодель | Добавлена как отдельный слой | Есть события аналитики, логика платного экрана, статьи себестоимости, формулы и сценарии подписка/image/video/hybrid. | Нужно уточнить модель после проверки реальных цен конкурентов, готовности платить, повторного возврата и стоимости привлечения. |
| Человечность текста | Лучше, но требует финальной редакторской вычитки перед клиентом | Текст на русском, с последовательным объяснением “мы думаем -> проверили -> вывод”. | Нужно еще сильнее убрать сухие англицизмы, служебные формулировки и повторы, чтобы документ читался как цельная стратегия. |

Главный вывод этого редакторского прохода: отчет уже можно читать как полноценную исследовательскую рамку, но до версии 10/10 ему не хватает трех вещей: ручного разбора главных конкурентов, уточнения финмодели на реальных ценах и 8-12 прототипных интервью. Следующий шаг должен быть не “еще больше данных вообще”, а точечное усиление этих трех доказательных слоев.
