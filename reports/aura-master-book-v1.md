...
AURA Product Master Plan

Единая книга проекта: стратегия, исследование, продуктовая спецификация, технический план, unit economics, GTM и sprint backlog.

Версия: v1
Проект: AURA
Центральная гипотеза: пользователь должен пройти петлю Episode -> Action -> Reset -> Reflection -> Avatar -> Tomorrow Hook и понять, что Life Canvas изменился из-за его действия.

| Пункт | Смысл |
| --- | --- |
| Что такое AURA | Mobile-first продукт, где личный контекст превращается в ежедневный эпизод, маленькое действие, reset, reflection и визуальный след в Life Canvas. |
| Главный риск | Не рынок, а понимание причинности Life Canvas. Если пользователь видит random AI image, продукт мертв. Если видит след своего действия, продукт жив. |
| Что входит в книгу | Исследование рынка и гипотез, MVP Specification, Technical Blueprint, Unit Economics, GTM Plan, PRD/Sprint Backlog. |
| Что не делаем сейчас | Не продолжаем бесконечный research. Следующий практический слой после master book: Figma wireframes и prototype interviews. |

## Как читать эту книгу

| Если есть | Читатель должен понять |
| --- | --- |
| 10 минут | Понять, что такое AURA и почему центральная петля важнее красивой AI-картинки. |
| 30 минут | Понять, как работает продукт: onboarding, season, episode, action, reset, reflection, avatar, paywall. |
| 60 минут | Понять, почему проект существует: рынок, соседние категории, конкуренты, белое пятно, аудитория. |
| 90 минут | Понять, как строить: архитектура, unit economics, GTM, sprint backlog, team plan. |

## Визуальный фреймворк проекта

| Шаг | Блок | Что происходит | Что может сломаться |
| --- | --- | --- | --- |
| 1 | Context | Дата рождения, состояние, запрос, тема сезона. | Если контекст слабый, episode звучит generic. |
| 2 | Episode | Смысл дня, конфликт, ресурс, риск. | Если episode не личный, пользователь не идет дальше. |
| 3 | Action | Маленький наблюдаемый шаг. | Если действие слишком тяжелое, loop не завершается. |
| 4 | Reset | Короткий мост через сопротивление. | Если reset отдельный и длинный, он мешает действию. |
| 5 | Reflection | Одна эмоция и одна строка доказательства. | Если reflection тяжела, пользователь не пишет. |
| 6 | Life Canvas | Визуальный след действия. | Если причинность не ясна, продукт становится image generator. |
| 7 | Tomorrow Hook | Причина вернуться завтра. | Если Day 2 не помнит Day 1, season распадается. |
...
Здесь мы разбираем рынок, соседние категории, конкурентов, гипотезы и причину, почему проект не нужно сводить к гороскопу, habit tracker или AI avatar generator.

| Слой | Смысл |
| --- | --- |
| Центральный вопрос главы | Здесь мы разбираем рынок, соседние категории, конкурентов, гипотезы и причину, почему проект не нужно сводить к гороскопу, habit tracker или AI avatar generator. |
| Красная нить | AURA существует только если пользователь понимает причинность: Life Canvas меняется из-за действия, а не потому что AI случайно нарисовал новую картинку. |
| Как читать | Сначала смотрим, зачем это важно, затем читаем содержательный блок, затем фиксируем решения. |
<!-- PAGEBREAK -->
## Вводная страница: рынок, гипотезы и белое пятно

| Вопрос | Ответ |
| --- | --- |
| Что разбираем | Мировой контекст, категории, конкуренты, аудитории, паттерны функций, деньги и стратегическое белое пятно. |
| Почему важно | До продукта нужно понять, не является ли AURA фантазией без рынка. Этот раздел отвечает: рынок и соседние спросы существуют. |
| Как влияет на продукт | Показывает, что AURA должна быть на пересечении meaning, action, memory and visual progress, а не копией одной категории. |
| Какой вывод должен быть сделан | Рынок достаточно живой, чтобы идти в прототип. Главный риск переезжает из research в product loop. |
...
## Вывод раздела: рынок и стратегическая логика

| Что фиксируем | Решение / влияние на продукт |
| --- | --- |
| Что узнали | Вокруг AURA есть большие соседние рынки: mobile apps, wellbeing, mindfulness, habit, AI companion, avatar/visual generation, astrology/self-discovery. |
| Что это значит | AURA не должна доказывать рынок с нуля; она должна доказать новую связку между смыслом, действием и визуальным следом. |
| Как влияет на продукт | Продуктовое ядро должно быть не “AI horoscope” и не “avatar app”, а personal life series. |
| Какие решения приняты | Остановить бесконечный research; перейти к MVP, прототипу, интервью и проверке Life Canvas causality. |
<!-- PAGEBREAK -->
# ГЛАВА 2

# Продуктовая петля и MVP

Здесь идея становится продуктом: что пользователь видит, какие экраны проходит, какие состояния меняются и что именно нужно проверить в первом MVP.

| Слой | Смысл |
| --- | --- |
| Центральный вопрос главы | Здесь идея становится продуктом: что пользователь видит, какие экраны проходит, какие состояния меняются и что именно нужно проверить в первом MVP. |
| Красная нить | AURA существует только если пользователь понимает причинность: Life Canvas меняется из-за действия, а не потому что AI случайно нарисовал новую картинку. |
| Как читать | Сначала смотрим, зачем это важно, затем читаем содержательный блок, затем фиксируем решения. |
<!-- PAGEBREAK -->
## Вводная страница: MVP Specification

| Вопрос | Ответ |
| --- | --- |
| Что разбираем | Полный путь пользователя, экраны, функции, состояния, сущности, API, acceptance criteria и границы MVP. |
| Почему важно | Инвестор и команда уже не спрашивают “есть ли рынок”. Они спрашивают “что конкретно строим”. |
| Как влияет на продукт | Фиксирует главный flow: Episode -> Action -> Reset -> Reflection -> Avatar -> Tomorrow Hook. |
| Какой вывод должен быть сделан | MVP должен проверять причинность Life Canvas и возврат на следующий день, а не весь большой vision. |
...
## Вывод раздела: MVP и пользовательский путь

| Что фиксируем | Решение / влияние на продукт |
| --- | --- |
| Что узнали | AURA уже можно описать как продукт: есть экраны, состояния, функции, API и критерии успеха. |
| Что это значит | Следующий практический шаг - low-fi Figma wireframes, не новый market research. |
| Как влияет на продукт | Все спорные функции проверяются вопросом: помогает ли это пользователю понять причинный avatar shift. |
| Какие решения приняты | No free daily video, no marketplace, no community, no AI friend in MVP; image-first Life Canvas обязателен. |
<!-- PAGEBREAK -->
# ГЛАВА 3

# Как AURA строить технически

Здесь продукт превращается в архитектуру: стек, сервисы, данные, AI, image generation, платежи, аналитика, admin и себестоимость.

| Слой | Смысл |
| --- | --- |
| Центральный вопрос главы | Здесь продукт превращается в архитектуру: стек, сервисы, данные, AI, image generation, платежи, аналитика, admin и себестоимость. |
| Красная нить | AURA существует только если пользователь понимает причинность: Life Canvas меняется из-за действия, а не потому что AI случайно нарисовал новую картинку. |
| Как читать | Сначала смотрим, зачем это важно, затем читаем содержательный блок, затем фиксируем решения. |
<!-- PAGEBREAK -->
## Вводная страница: Technical Blueprint и Unit Economics

| Вопрос | Ответ |
| --- | --- |
| Что разбираем | Frontend, backend, DB, storage, AI, image, payments, analytics, admin, unit economics, cost controls, risks. |
| Почему важно | Видео, AI и генерация могут быстро убить экономику продукта, если не задать ограничения до разработки. |
| Как влияет на продукт | Архитектура должна поддерживать быстрый MVP, cost logging и безопасные AI outputs. |
| Какой вывод должен быть сделан | Строим mobile-first, image-first, analytics-first. Видео только later/premium/token. |
...
## Вывод раздела: технологии и экономика

| Что фиксируем | Решение / влияние на продукт |
| --- | --- |
| Что узнали | MVP технически реализуем, но экономика требует жесткого контроля изображений и запрета бесплатного ежедневного видео. |
| Что это значит | Нужно считать cost per completed loop и cost per returned user, а не только стоимость разработки. |
| Как влияет на продукт | Архитектурные решения должны фиксировать события, стоимость генераций, prompt versions и safety status. |
| Какие решения приняты | React Native/Expo, NestJS, Postgres/Supabase, OpenAI GPT-4.1 mini, FLUX image layer, RevenueCat, analytics and admin controls. |
<!-- PAGEBREAK -->
# ГЛАВА 4

# Как получить первых пользователей

Здесь мы перестаем думать про абстрактный маркетинг и описываем конкретный путь к первым 100 и 1000 пользователям.

| Слой | Смысл |
| --- | --- |
| Центральный вопрос главы | Здесь мы перестаем думать про абстрактный маркетинг и описываем конкретный путь к первым 100 и 1000 пользователям. |
| Красная нить | AURA существует только если пользователь понимает причинность: Life Canvas меняется из-за действия, а не потому что AI случайно нарисовал новую картинку. |
| Как читать | Сначала смотрим, зачем это важно, затем читаем содержательный блок, затем фиксируем решения. |
<!-- PAGEBREAK -->
## Вводная страница: GTM Plan

| Вопрос | Ответ |
| --- | --- |
| Что разбираем | Positioning, первые 100 пользователей, первые 1000, каналы, контент, creator outreach, landing variants, hooks, objections. |
| Почему важно | MVP понятен, но без правильного GTM мы не узнаем, кому продукт действительно нужен. |
| Как влияет на продукт | Первые пользователи должны не просто скачать, а пройти loop и объяснить Life Canvas causality. |
| Какой вывод должен быть сделан | Первые 100 пользователей не покупаются рекламой; они добываются вручную через интервью, прототип и concierge. |

Этот документ отвечает не на вопрос “есть ли рынок”, а на вопрос “как получить первых пользователей и доказать, что петля Episode -> Action -> Reset -> Avatar -> Return Tomorrow работает в реальности”. GTM для AURA на первом этапе должен быть не про масштабный media buying, а про быстрые циклы: оффер, прототип, интервью, ручная активация, первые оплаты, повторяемый контент.
...
| Week 4 | Paid willingness and cohort readout | Paywall test, annual/monthly anchor, 7-day recap interviews, MVP go/no-go. | Trial intent, D7 completion, top paid reasons, kill reasons. |
...
| 27 | Objection | Почему мы не делаем social network в MVP. | Agree/disagree. |
...
| 21 | Мы убрали видеоаватары из MVP, потому что они могут убить экономику. | Founder transparency |
...
| Я не хочу грузить фото лица | В MVP face upload не нужен; Life Canvas может быть символическим. | Privacy reassurance. |
...
| AI будет писать общие фразы | Это главный риск MVP; поэтому тестируем relevance и собираем feedback. | Build in public. |
| Я брошу через день | MVP специально проверяет D1/D7; comeback flow не стыдит за пропуск. | Retention content. |
...
| End of 30 days | Decision meeting. | Founder/team | Build MVP, pivot positioning, or stop. |
...
## Вывод раздела: GTM и запуск

| Что фиксируем | Решение / влияние на продукт |
| --- | --- |
| Что узнали | Запуск должен быть founder-led, cohort-based and learning-first. |
| Что это значит | Реклама до retention и causality comprehension опасна: можно купить шум и не понять продукт. |
| Как влияет на продукт | Контент и onboarding должны объяснять новую категорию: personal life series, not horoscope, not random avatar. |
| Какие решения приняты | 20 interviews, 30 concierge users, 100 warm users, 1000 через short video/micro creators only after core signals. |
<!-- PAGEBREAK -->
# ГЛАВА 5

# Как превратить план в работу команды

Здесь master book становится операционным документом: спринты, задачи, часы, стоимость, зависимости и критерии приемки.

| Слой | Смысл |
| --- | --- |
| Центральный вопрос главы | Здесь master book становится операционным документом: спринты, задачи, часы, стоимость, зависимости и критерии приемки. |
| Красная нить | AURA существует только если пользователь понимает причинность: Life Canvas меняется из-за действия, а не потому что AI случайно нарисовал новую картинку. |
| Как читать | Сначала смотрим, зачем это важно, затем читаем содержательный блок, затем фиксируем решения. |
<!-- PAGEBREAK -->
## Вводная страница: PRD / Sprint Backlog

| Вопрос | Ответ |
| --- | --- |
| Что разбираем | Пять спринтов, backlog, часы, стоимость, зависимости, critical path, Definition of Done, team plan. |
| Почему важно | Команде нужен не вдохновляющий текст, а понятный план сборки MVP. |
| Как влияет на продукт | Каждая задача привязана к центральной петле и не расширяет MVP за пределы проверки. |
| Какой вывод должен быть сделан | После wireframes можно начинать Sprint 1; research свою работу выполнил. |
...
## Вывод раздела: план разработки

| Что фиксируем | Решение / влияние на продукт |
| --- | --- |
| Что узнали | Есть реалистичный путь из пяти спринтов к soft-launch MVP. |
| Что это значит | Проект можно переводить в дизайн и разработку без нового research loop. |
| Как влияет на продукт | Scope остается защищенным: никакого marketplace/community/video-first до проверки Day 1/Day 2. |
| Какие решения приняты | Следующий обязательный артефакт - Figma wireframes для 10 ключевых экранов. |
<!-- PAGEBREAK -->
# ГЛАВА 6

# Финальная рамка принятия решений

Здесь фиксируем, что считать успехом, что считать провалом и что делать сразу после master book.

| Слой | Смысл |
| --- | --- |
| Центральный вопрос главы | Здесь фиксируем, что считать успехом, что считать провалом и что делать сразу после master book. |
| Красная нить | AURA существует только если пользователь понимает причинность: Life Canvas меняется из-за действия, а не потому что AI случайно нарисовал новую картинку. |
| Как читать | Сначала смотрим, зачем это важно, затем читаем содержательный блок, затем фиксируем решения. |
<!-- PAGEBREAK -->
## Вводная страница: финальная система решений

| Вопрос | Ответ |
| --- | --- |
| Что разбираем | Go/no-go logic, metrics, kill criteria, immediate next steps. |
| Почему важно | Большой документ бесполезен, если после него команда снова начинает спорить о рынке вместо проверки прототипа. |
| Как влияет на продукт | Все решения должны вести к проверке: понимает ли пользователь причинный Life Canvas. |
| Какой вывод должен быть сделан | AURA готова к wireframes, prototype, interviews and first cohort. |

## Финальный decision framework

| Вопрос | Решение |
| --- | --- |
| Что строим? | Mobile-first MVP personal life series: episode, action, reset, reflection, Life Canvas, tomorrow hook. |
| Для кого? | Spiritual self-improvers, habit/progress users, avatar/future-self users, journaling/reset users. |
| Почему может выстрелить? | AURA соединяет смысл, действие, память и визуальный прогресс в одну петлю. |
| Почему может провалиться? | Если Life Canvas воспринимается как случайная AI-картинка, а не след действия. |
| Что проверяем первым? | Avatar causality comprehension, completed first loop, D1 return, D7 season completion, paid intent. |
| Что запрещено в MVP? | Free daily video, marketplace, community, social network, AR/metaverse, public UGC, heavy AI friend. |
| Что делать дальше? | Figma low-fi wireframes -> clickable prototype -> 20 interviews -> 30 concierge users -> first cohort readout. |


## Вывод раздела: вся книга

| Что фиксируем | Решение / влияние на продукт |
| --- | --- |
| Что узнали | AURA прошла путь от идеи к системе: рынок понятен, продукт описан, технология рассчитана, GTM собран, разработка разложена. |
| Что это значит | Research package выполнил свою работу. Следующий риск находится внутри прототипа. |
| Как влияет на продукт | Все дальнейшие обсуждения должны возвращаться к одной проверке: понимает ли пользователь, почему изменился Life Canvas. |
| Какие решения приняты | Начинаем с Figma wireframes и prototype interviews. Не продолжаем расширять research без пользовательских сигналов. |
