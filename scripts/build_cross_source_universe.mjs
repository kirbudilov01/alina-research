...
function sourceGroup(sourceKind, sourceBucket, platform) {
  ...
  if (/company|positioning|public web|website|paywall/.test(text)) return 'company_positioning';
...
const inputs = [
  ...
  ['data_raw/company_positioning_raw.csv', csv('data_raw/company_positioning_raw.csv')]
];
...
lines.push('## Purpose');
lines.push('');
lines.push('This layer normalizes the major source-native collections into one auditable universe. It keeps raw provenance while producing a cross-source deduplicated view across core app-store rows, itch.io, Steam, Mac desktop store, Chrome Web Store search/detail pages, Reddit forum mention discovery, and known company/public-web positioning pages.');
lines.push('');
lines.push('## Summary');
lines.push('');
