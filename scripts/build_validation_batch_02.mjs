...
const OUT = 'data_processed/validation_batch_02_index.csv';
const OUT_DOC = 'docs/decision/validation-batch-02-v1.md';
...
function noteFor(row) {
  const lines = [];
  lines.push(`# Validation Batch 02 Note: ${row.command_id}`);
  lines.push('');
...
const rows = p0Rows.map((row, index) => {
  const dir = `${ROOT}/${row.lane}`;
  fs.mkdirSync(dir, { recursive: true });
  const notePath = `${dir}/batch02_${String(index + 1).padStart(2, '0')}__${row.command_id}__${slug(row.target)}__notes.md`;
...
const lines = [];
lines.push('# Validation Batch 02 V1');
lines.push('');
lines.push(`Generated: ${new Date().toISOString()}`);
lines.push('');
lines.push('## Purpose');
lines.push('');
lines.push('Batch 02 pre-creates evidence notes for every non-blocker P0 command. It expands validation from the six blocker gates into the full P0 breadth across manual walkthrough, paid-flow signoff, ICP interviews, prototype sessions, and scorecard gates.');
lines.push('');
lines.push('## Summary');
lines.push('');
...
lines.push('');
lines.push('- Fill Batch 01 blocker notes first when a blocker and a Batch 02 row compete for attention.');
lines.push('- Fill the note file before editing linked source CSVs, then keep the note and source CSV verdict aligned.');
lines.push('- Do not change H1-H6 verdicts until evidence notes, capture sheets, and source CSVs agree.');
lines.push('- If a downgrade/kill gate triggers, update report/PDF caveats in the same commit.');
lines.push('');
lines.push('## Files');
lines.push('');
lines.push(`- \`${OUT}\``);
