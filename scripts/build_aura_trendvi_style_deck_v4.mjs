...
const WORKSPACE = path.join(ROOT, 'tmp', 'presentations', THREAD_ID, 'aura-trendvi-style-deck-v4');
...
const PLAN_OUT = path.join(ROOT, 'reports', 'aura-trendvi-style-deck-v4.md');
const SOURCE_NOTES = path.join(ROOT, 'reports', 'aura-trendvi-style-deck-v4-sources.md');
const PPTX_OUT = path.join(OUTPUT_DIR, 'AURA_TRENDVI_STYLE_DECK_V4.pptx');
const CONTACT_SHEET = path.join(OUTPUT_DIR, 'AURA_TRENDVI_STYLE_DECK_V4_CONTACT_SHEET.png');
...
const slides = [
  ...
  },
  {
    ...
    note: 'Все английские сокращения в презентации нужны только как индустриальный язык. Для решения читаем русскую колонку: что проверяем и почему это важно.'
  },
  {
  ...
  {
    title: 'Где могут придраться — и как отвечать',
    type: 'objectionMap',
    rows: [
      ['Кто спросит', 'Вопрос / уязвимость', 'Слабое место', 'Ответ в V4'],
      ['Клиент', '“Что именно мы делаем в первую фазу?”', 'можно спутать проверку с полным продуктом', 'Фаза 1 = €4k-€10k: визуалы, прототип, лендинг, 30-user cohort'],
      ['Клиент', '“Почему не сразу видео/аватар каждый день?”', 'видео выглядит как главный wow', 'ежедневная петля image-first; видео только premium/token из-за COGS'],
      ['Инвестор', '“Откуда цифры конкурентов?”', 'часть данных — public proxies, не audited P&L', 'помечаем как directional benchmarks и используем как guardrails'],
      ['Инвестор', '“Почему €80k-€150k, если модель растет до 150k MAU?”', 'в V3 смешивались pre-scale и scale path', 'в V4 разделено: €80k-€150k до scale, €500k+ для агрессивного 150k MAU path'],
      ['Продакт', '“Что доказывает, что это не AI-картинка?”', 'главный риск — случайность Canvas', 'главный gate: >50% пользователей объясняют Canvas causality'],
      ['Маркетинг', '“Можно ли покупать трафик?”', 'CAC без retention опасен', 'paid acquisition только после D1, paid intent и free COGS tracking']
    ],
    note: 'V4 не прячет слабые места. Она показывает, какие вопросы нужно проверить до дорогой разработки.'
  },
  {
    title: 'Готовность к сдаче клиенту',
    type: 'readinessCheck',
    rows: [
      ['Блок', 'Статус', 'Что уже есть', 'Что остается после презентации'],
      ['Продуктовая логика', 'готово', 'понятная петля Episode → Action → Reset → Canvas', 'сделать 3-5 настоящих visual examples'],
      ['Финансовая модель', 'готово как рабочая гипотеза', 'цена, COGS, free user, CAC, runway, сценарии', 'пересчитать после первых usage/logging данных'],
      ['GTM', 'готово для первой проверки', 'creator-led, micro tests, landing, 30-day validation', 'не масштабировать рекламу до paid intent'],
      ['Технологии', 'готово для выбора MVP-стека', 'image-first, capped video, API cost guardrails', 'выбрать конкретных providers перед build'],
      ['Инвестиционная история', 'достаточно для разговора', 'рынок, белое пятно, экономика, gates', 'добавить traction после 30-user cohort']
    ],
    note: 'Вывод: презентацию можно показывать как стратегию проверки. Нельзя продавать ее как доказанный бизнес до прототипа и cohort data.'
  },
  {
    title: 'Roadmap роста',
    type: 'roadmap',
...
function bodyFor(slide, n) {
  ...
  ctx.addText(slide, { text: 'Продуктовая / инвесторская презентация · V4', left: 390, top: 382, width: 650, height: 34, fontSize: 24, fontFace: 'Arial', color: C.muted, align: 'center' });
`;
  }
...
`;
  ...
  ctx.addText(slide, { text: 'Принцип V4: сначала смысл и решение, потом доказательства, таблицы и условия go/no-go.', left: 230, top: 578, width: 720, height: 22, fontSize: 19, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
`;
  }
...
`;
  ...
  if (slide.type === 'objectionMap') {
    return `
  title(slide, ctx, '${esc(slide.title)}');
  ctx.addText(slide, { text: 'Это pre-mortem: где презентацию могут атаковать и какой честный ответ уже встроен в историю.', left: 102, top: 114, width: 980, height: 32, fontSize: 20, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
  ${table(slide.rows, [150, 260, 285, 375], 54, 165, 56)}
  ctx.addText(slide, { text: '${esc(slide.note)}', left: 115, top: 636, width: 950, height: 20, fontSize: 15, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
`;
  }
  ...
  if (slide.type === 'readinessCheck') {
    return `
  title(slide, ctx, '${esc(slide.title)}');
  ctx.addShape(slide, { left: 80, top: 118, width: 1040, height: 48, fill: C.purpleLight, line: { style: 'solid', fill: C.line, width: 1 } });
  ctx.addText(slide, { text: 'Вердикт: можно показывать клиенту как план проверки и основу для следующей фазы, но не как доказанный бизнес без cohort data.', left: 105, top: 131, width: 990, height: 20, fontSize: 18, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
  ${table(slide.rows, [175, 190, 335, 360], 54, 190, 56)}
