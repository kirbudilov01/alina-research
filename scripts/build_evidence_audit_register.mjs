...
const competitorUniverseStatus = crossSourceRaw.length >= 50000 && crossSourceDedup.length >= 30000
...
const competitorUniverseGap = crossSourceRaw.length >= 50000 && crossSourceDedup.length >= 30000
  ? 'Raw 50k source scale is met; dedup 30k+ and the 30k-40k working band are met; dedup 50k remains open and should not be overclaimed.'
  : (crossSourceDedup.length >= 30000
...
const rows = [
  ...
  },
  {
    ...
    quantitative_evidence: `niches=${Object.keys(countBy(expanded, 'niche')).length}; source_kinds=${Object.keys(countBy(expanded, 'source_kind')).length}; source_scale_milestones=${sourceScaleMilestone.length}; cross_source_summary_rows=${crossSourceSummary.length}; coverage_cells=${crossSourceCoverage.length}; coverage_strong=${crossSourceCoverage.filter(row => row.coverage_band === 'strong_coverage').length}; coverage_medium=${crossSourceCoverage.filter(row => row.coverage_band === 'medium_coverage').length}; p0_external_rows=${p0External.length}; p0_external_usable=${p0ExternalUsable.length}; itch_rows=${itchRows.length}; itch_ok=${itchOk.length}; steam_tag_rows=${steamTagRows.length}; steam_tag_ok=${steamTagOk.length}; steam_deep_tag_rows=${steamDeepTagRows.length}; steam_deep_tag_ok=${steamDeepTagRows.filter(row => row.collection_status === 'ok').length}; desktop_store_rows=${desktopStoreRows.length}; desktop_store_ok=${desktopStoreOk.length}; chrome_extension_rows=${chromeExtensionRows.length}; chrome_extension_ok=${chromeExtensionOk.length}; reddit_mention_rows=${redditMentionRows.length}; reddit_mention_ok=${redditMentionOk.length}; reddit_mention_summary_rows=${redditMentionSummary.length}; reddit_signal_rows=${redditMentionSignals.length}; reddit_signal_groups=${Object.keys(countBy(redditMentionSignals, 'signal_group')).length}; reddit_known_signal_rows=${redditMentionKnownSignalRows.length}; reddit_app_summary_rows=${redditMentionAppSummary.length}; reddit_manual_queue_rows=${redditManualReadingQueue.length}; reddit_manual_p0=${redditManualP0.length}; reddit_manual_p1=${redditManualP1.length}; reddit_manual_capture_rows=${redditManualCaptureSheet.length}; reddit_manual_capture_p0=${redditManualCaptureP0.length}; chrome_detail_ok=${chromeExtensionDetailOk.length}; chrome_strong_adjacent=${chromeExtensionStrong.length}; chrome_priority_mechanics=${chromeMechanicPriority.length}`,
    evidence_files: 'data_raw/expanded/all_expanded_raw.csv;data_raw/expanded/all_expanded_dedup.csv;data_raw/expanded/p0_external_sources_raw.csv;data_raw/expanded_itch_raw.csv;data_raw/expanded_steam_tags_raw.csv;data_raw/expanded_steam_deep_tags_raw.csv;data_raw/expanded_desktop_store_raw.csv;data_raw/expanded_chrome_extensions_raw.csv;data_raw/expanded_reddit_competitor_mentions_raw.csv;data_raw/chrome_extension_detail_raw.csv;data_processed/cross_source_universe_raw_index.csv;data_processed/cross_source_universe_raw_parts/part_*.csv;data_processed/cross_source_universe_dedup.csv;data_processed/cross_source_universe_summary.csv;data_processed/cross_source_coverage_matrix.csv;data_processed/source_scale_milestone.csv;data_processed/p0_external_source_summary.csv;data_processed/itch_source_summary.csv;data_processed/steam_tag_source_summary.csv;data_processed/steam_deep_tag_source_summary.csv;data_processed/desktop_store_source_summary.csv;data_processed/chrome_webstore_source_expansion_summary.csv;data_processed/reddit_competitor_mentions_summary.csv;data_processed/reddit_mention_signal_matrix.csv;data_processed/reddit_mention_app_summary.csv;data_processed/reddit_manual_reading_queue.csv;data_processed/reddit_manual_reading_prompt_bank.csv;data_processed/reddit_manual_reading_capture_sheet.csv;data_processed/chrome_extension_fit_matrix.csv;data_processed/chrome_extension_mechanic_battlecards.csv;data_processed/competitor_feature_matrix.csv;docs/competitive/cross-source-universe-v1.md;docs/competitive/cross-source-coverage-matrix-v1.md;docs/competitive/source-scale-milestone-v1.md;docs/competitive/expanded-source-map.md;docs/competitive/p0-external-source-collection-v1.md;docs/competitive/itch-source-expansion-v1.md;docs/competitive/steam-tag-expansion-v1.md;docs/competitive/steam-deep-tag-increment-v1.md;docs/competitive/desktop-store-expansion-v1.md;docs/competitive/chrome-webstore-source-expansion-v1.md;docs/audience/reddit-competitor-mentions-v1.md;docs/audience/reddit-mention-signal-matrix-v1.md;docs/audience/reddit-manual-reading-queue-v1.md;docs/audience/reddit-manual-reading-capture-sheet-v1.md;docs/competitive/chrome-extension-detail-enrichment-v1.md;docs/competitive/chrome-extension-mechanic-battlecards-v1.md',
  ...
  },
  {
    ...
    primary_metric: `${webVisualConfirmed.length}/${webPaywallVisualAdjudication.length} screenshots confirm visible public pricing; ${webVisualPartial.length} partial paid-surface examples; ${paidFlowLocalSignoff.length} local signoff rows`,
    quantitative_evidence: `web_domains=${webPaywalls.length}; screenshot_captured=${screenshots.filter(row => row.screenshot_status === 'captured').length}; ocr_confirmed=${webConfirmedPricing.length}; visual_adjudication_rows=${webPaywallVisualAdjudication.length}; visual_summary_markets=${webPaywallVisualAdjudicationSummary.length}; visual_confirmed=${webVisualConfirmed.length}; visual_partial=${webVisualPartial.length}; visual_weakened=${webVisualWeakened.length}; weakens_signal=${webWeakens.length}; paid_flow_local_signoff=${paidFlowLocalSignoff.length}`,
    evidence_files: 'data_processed/web_paywall_signal_matrix.csv;data_processed/web_paywall_screenshot_validation.csv;data_processed/web_paywall_screenshot_interpretation.csv;data_processed/web_paywall_visual_adjudication.csv;data_processed/web_paywall_visual_adjudication_summary.csv;data_processed/paid_flow_local_signoff.csv;docs/competitive/web-paywall-screenshot-interpretation-v1.md;docs/competitive/web-paywall-visual-adjudication-v1.md;docs/market/paid-flow-local-signoff-v1.md;output/paywall_screenshots/*.png',
    ...
    key_gap: 'Most web signals remain ambiguous, not found, parent-company pages, login-gated, or require broader human/in-app inspection; current local signoff covers only the first two confirmed/partial products.',
  ...
  },
  {
    ...
    confidence: 'medium',
    primary_metric: `${audience.length} audience signal rows; ${communityReferralRows.length} community/referral rows; ${redditMentionSignals.length} coded Reddit mention rows; ${redditManualReadingQueue.length} Reddit manual-read queue rows; ${redditManualCaptureSheet.length} Reddit capture rows; ${icpSegments.length} ICP segment hypotheses; ${icpValidationPlan.length} ICP validation tests; ${icpRecruitingBridge.length} ICP recruiting bridge rows`,
    quantitative_evidence: `reviews=${reviews.length}; review_apps=${reviewApps}; review_signals=${reviewSignals.length}; review_clusters=${reviewClusters.length}; community_referral_rows=${communityReferralRows.length}; community_referral_signals=${communityReferralSummary.length}; forum_quote_rows=${forumQuotes.length}; reddit_signal_rows=${redditMentionSignals.length}; reddit_signal_groups=${Object.keys(countBy(redditMentionSignals, 'signal_group')).length}; reddit_medium_plus_signals=${redditMediumPlusSignals.length}; reddit_app_summary_rows=${redditMentionAppSummary.length}; reddit_manual_queue_rows=${redditManualReadingQueue.length}; reddit_manual_p0=${redditManualP0.length}; reddit_manual_p1=${redditManualP1.length}; reddit_prompt_rows=${redditManualPromptBank.length}; reddit_manual_capture_rows=${redditManualCaptureSheet.length}; reddit_manual_capture_p0=${redditManualCaptureP0.length}; reddit_manual_capture_completed=${redditManualCaptureCompleted.length}; icp_segments=${icpSegments.length}; strong_icp=${strongIcpSegments.length}; icp_validation_tests=${icpValidationPlan.length}; icp_recruiting_bridge_rows=${icpRecruitingBridge.length}; icp_recruiting_message_rows=${icpRecruitingMessages.length}`,
    evidence_files: 'data_processed/audience_signal_matrix.csv;data_raw/app_store_top_candidate_reviews.csv;data_processed/review_signal_matrix.csv;data_processed/review_jtbd_cluster_summary.csv;data_processed/community_referral_signal_rows.csv;data_processed/community_referral_summary.csv;data_processed/forum_quote_coding_matrix.csv;data_processed/reddit_mention_signal_matrix.csv;data_processed/reddit_mention_app_summary.csv;data_processed/reddit_manual_reading_queue.csv;data_processed/reddit_manual_reading_prompt_bank.csv;data_processed/reddit_manual_reading_capture_sheet.csv;data_processed/icp_segment_matrix.csv;data_processed/icp_validation_test_plan.csv;data_processed/icp_recruiting_bridge.csv;data_processed/icp_recruiting_message_bank.csv;docs/audience/review-language-synthesis-v1.md;docs/audience/community-referral-evidence-v1.md;docs/audience/forum-quote-coding-v1.md;docs/audience/reddit-mention-signal-matrix-v1.md;docs/audience/reddit-manual-reading-queue-v1.md;docs/audience/reddit-manual-reading-capture-sheet-v1.md;docs/audience/icp-segment-matrix-v1.md;docs/audience/icp-validation-packet-v1.md;docs/audience/icp-recruiting-bridge-v1.md',
