...
const rows = [
  ...
  },
  {
    ...
    confidence: 'medium',
    primary_metric: `${audience.length} audience signal rows; ${communityReferralRows.length} community/referral rows; ${icpSegments.length} ICP segment hypotheses; ${icpValidationPlan.length} ICP validation tests; ${icpRecruitingBridge.length} ICP recruiting bridge rows`,
    quantitative_evidence: `reviews=${reviews.length}; review_apps=${reviewApps}; review_signals=${reviewSignals.length}; review_clusters=${reviewClusters.length}; community_referral_rows=${communityReferralRows.length}; community_referral_signals=${communityReferralSummary.length}; forum_quote_rows=${forumQuotes.length}; icp_segments=${icpSegments.length}; strong_icp=${strongIcpSegments.length}; icp_validation_tests=${icpValidationPlan.length}; icp_recruiting_bridge_rows=${icpRecruitingBridge.length}; icp_recruiting_message_rows=${icpRecruitingMessages.length}`,
    evidence_files: 'data_processed/audience_signal_matrix.csv;data_raw/app_store_top_candidate_reviews.csv;data_processed/review_signal_matrix.csv;data_processed/review_jtbd_cluster_summary.csv;data_processed/community_referral_signal_rows.csv;data_processed/community_referral_summary.csv;data_processed/forum_quote_coding_matrix.csv;data_processed/icp_segment_matrix.csv;data_processed/icp_validation_test_plan.csv;data_processed/icp_recruiting_bridge.csv;data_processed/icp_recruiting_message_bank.csv;docs/audience/review-language-synthesis-v1.md;docs/audience/community-referral-evidence-v1.md;docs/audience/forum-quote-coding-v1.md;docs/audience/icp-segment-matrix-v1.md;docs/audience/icp-validation-packet-v1.md;docs/audience/icp-recruiting-bridge-v1.md',
    strongest_support: 'Reviews, community/referral mentions, and forum snippets converge on daily anchors, visible progress, emotional support, pricing sensitivity, accountability/community language, and safety boundaries; the ICP segment matrix and recruiting bridge convert those signals into testable segment, channel, screener, prototype, WTP, and evidence-capture rows.',
