...
const WORKSPACE = path.join(ROOT, 'tmp', 'presentations', THREAD_ID, 'aura-trendvi-style-deck-v3');
...
const PLAN_OUT = path.join(ROOT, 'reports', 'aura-trendvi-style-deck-v3.md');
const SOURCE_NOTES = path.join(ROOT, 'reports', 'aura-trendvi-style-deck-v3-sources.md');
const PPTX_OUT = path.join(OUTPUT_DIR, 'AURA_TRENDVI_STYLE_DECK_V3.pptx');
const CONTACT_SHEET = path.join(OUTPUT_DIR, 'AURA_TRENDVI_STYLE_DECK_V3_CONTACT_SHEET.png');
...
const slides = [
  ...
  },
  {
    ...
    lanes: [
      ...
      ['3', 'Деньги', 'pricing, COGS, free user, break-even'],
  ...
  },
  {
    ...
    type: 'metricCompass',
    groups: [
      ['Product', 'Activation', 'D1/D7', 'Canvas causality', 'понимает ли человек ценность'],
      ['Money', 'Paid intent', 'Conversion', 'ARPPU / ARPMAU', 'есть ли платная модель'],
      ['Cost', 'Free COGS', 'Paid COGS', 'Video COGS', 'не съедает ли AI маржу'],
      ['Growth', 'CAC signup', 'CAC activated', 'Creator proof', 'можно ли приводить людей']
    ],
  ...
  },
  {
    ...
    rows: [
      ...
      ['D1 retention', '>20%', '>30%', 'daily episode вызывает возврат'],
      ['Paid intent', '>5%', '>8-10%', 'можно переходить к paywall / pricing test'],
      ['Paid COGS', '<$4/мес', '<$3/мес', 'Visual Plus не убивает маржу'],
      ['CAC activated', '<$1.00', '<$2.00 при strong LTV', 'можно начинать маленькие paid tests']
    ],
    note: 'Эти пороги превращают презентацию в decision system: не “верим в идею”, а проверяем конкретные gates.'
  },
  {
  ...
  },
  {
    ...
    tiles: [
      ...
      ['Year 1 runway', '€80k-€150k', 'реалистичный бюджет до сильной MVP-проверки'],
      ...
      ['Go/No-Go', 'D1 >20% / intent >5%', 'иначе режем scope или останавливаем']
    ],
    flow: [
...
function bodyFor(slide, n) {
  ...
  ctx.addText(slide, { text: 'Продуктовая / инвесторская презентация · V3', left: 390, top: 382, width: 650, height: 34, fontSize: 24, fontFace: 'Arial', color: C.muted, align: 'center' });
`;
  }
...
`;
  ...
  ctx.addText(slide, { text: 'Принцип V2: сначала смысл и решение, потом доказательства и таблицы.', left: 230, top: 578, width: 720, height: 22, fontSize: 19, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
`;
  }
...
`;
  ...
  ctx.addText(slide, { text: '${esc(t[1])}', left: ${x + 140}, top: ${y + 12}, width: 150, height: 32, fontSize: 25, bold: true, fontFace: 'Montserrat', color: C.ink, align: 'right', fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(t[2])}', left: ${x + 18}, top: ${y + 58}, width: 284, height: 40, fontSize: 15, fontFace: 'Arial', color: C.ink, fit: 'shrink' });`;
  }).join('\n')}
...
function metricText(slide) {
  ...
  if (/конкурент|astrology|mindfulness|companion|avatar|лидер/i.test(title)) return 'Смотреть на: revenue / MAU / payers / conversion. Смысл: какие платные поведения уже доказаны рынком.';
  if (/pricing|paywall|платят|подписк/i.test(title)) return 'Смотреть на: цена, что бесплатно, что платно, где появляется premium. Смысл: за какой value moment пользователь платит.';
  ...
  if (/финансов|помесяч|кварталь|годов|cash|runway|break-even|экономик/i.test(title)) return 'Смотреть на: выручка, расходы, runway, break-even. Смысл: сколько денег нужно до решения go/no-go.';
  if (/маркет|воронк|hooks|GTM/i.test(title)) return 'Смотреть на: CAC, activation, paid intent, creator proof. Смысл: можно ли привлекать пользователей без сжигания бюджета.';
  if (/Roadmap|план|Scope|Риски/i.test(title)) return 'Смотреть на: gate, риск, решение. Смысл: что делаем сейчас, что запрещено тащить в MVP.';
