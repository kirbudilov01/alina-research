...
const OUT = 'data_processed/validation_batch_01_index.csv';
const OUT_DOC = 'docs/decision/validation-batch-01-v1.md';
...
function noteFor(row) {
  const lines = [];
  lines.push(`# Validation Batch 01 Note: ${row.command_id}`);
  lines.push('');
...
const blockers = commands.filter(row => row.priority === 'P0_blocker');

const rows = blockers.map((row, index) => {
  const dir = `${ROOT}/${row.lane}`;
  fs.mkdirSync(dir, { recursive: true });
  const notePath = `${dir}/batch01_${String(index + 1).padStart(2, '0')}__${row.command_id}__${slug(row.target)}__notes.md`;
...
const lines = [];
lines.push('# Validation Batch 01 V1');
lines.push('');
lines.push(`Generated: ${new Date().toISOString()}`);
lines.push('');
lines.push('## Purpose');
lines.push('');
lines.push('Batch 01 pre-creates evidence notes for the P0 blocker rows. These are the first validation files to fill before any H1-H6 hypothesis can move out of hold/validate.');
lines.push('');
lines.push('## Summary');
lines.push('');
...
lines.push('');
lines.push('- Fill the note file first, then update the linked source CSV and capture sheet.');
lines.push('- Do not change H1-H6 verdicts until the evidence note and source CSV agree.');
