...
report.push(`- Russian narrative report: ${fs.existsSync('output/pdf/alina-russian-narrative-report-v1.pdf') ? 'generated as a sequential Russian-language narrative PDF' : 'not generated yet'}; argument map: ${fs.existsSync('data_processed/russian_narrative_evidence_map.csv') ? 'generated' : 'not generated yet'}; Russian market deep dives: ${russianMarketDeepDives.length} market rows; Russian competitor battlecards: ${russianCompetitorBattlecards.length} P0 cards; Russian ICP battlecards: ${russianIcpBattlecards.length} segment cards; Russian product-loop cards: ${russianProductLoopCards.length} screen cards; Russian validation gate cards: ${russianValidationGateCards.length} H1-H6 cards; Russian P0 execution packet: ${russianP0ExecutionPacket.length} tranches; Russian claim appendix: ${russianClaimAppendix.length} claim rows; Russian source provenance: ${russianSourceProvenance.length} provenance rows; Russian fieldbook: ${fs.existsSync('data_processed/russian_validation_fieldbook.csv') ? 'generated' : 'not generated yet'}.`);
