...
const WORKSPACE = path.join(ROOT, 'tmp', 'presentations', THREAD_ID, 'aura-trendvi-style-deck-v8-main');
...
const PLAN_OUT = path.join(ROOT, 'reports', 'aura-trendvi-style-deck-v8-main.md');
const SOURCE_NOTES = path.join(ROOT, 'reports', 'aura-trendvi-style-deck-v8-main-sources.md');
const PPTX_OUT = path.join(OUTPUT_DIR, 'AURA_TRENDVI_STYLE_DECK_V8_MAIN.pptx');
const CONTACT_SHEET = path.join(OUTPUT_DIR, 'AURA_TRENDVI_STYLE_DECK_V8_MAIN_CONTACT_SHEET.png');
...
function writePlan() {
  const lines = ['# AURA Trendvi-Style Product Deck V8 Main', '', 'Deck mode: optimized client-facing AURA product deck. V8 Main is the client-facing premium story: product, competitor meaning, money logic, GTM and launch decision with fewer table-heavy moments.', ''];
  lines.push('## Client-facing main deck');
  lines.push('');
  lines.push(`- Main story: ${slides.length} slides including the competitor implications bridge.`);
  lines.push('- Keeps decisions, money logic, launch gates and client-readable competitor meaning.');
  lines.push('- Dense proof tables, detailed cost benchmarks, monthly models and marketing hooks are intentionally excluded from this client-facing version.');
  lines.push('');
