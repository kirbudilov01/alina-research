...
const WORKSPACE = path.join(ROOT, 'tmp', 'presentations', THREAD_ID, 'aura-trendvi-style-deck-v8');
...
const PLAN_OUT = path.join(ROOT, 'reports', 'aura-trendvi-style-deck-v8.md');
const SOURCE_NOTES = path.join(ROOT, 'reports', 'aura-trendvi-style-deck-v8-sources.md');
const PPTX_OUT = path.join(OUTPUT_DIR, 'AURA_TRENDVI_STYLE_DECK_V8.pptx');
const CONTACT_SHEET = path.join(OUTPUT_DIR, 'AURA_TRENDVI_STYLE_DECK_V8_CONTACT_SHEET.png');
...
const byTitle = new Map(allSlides.map(slide => [slide.title, slide]));

const mainSlideTitles = [
  'AURA',
  'Короткий ответ: что мы строим',
  'Компас показателей',
  'Что уже доказали лидеры',
  'Новый формат находится на пересечении',
  'Пользовательская проблема → ответ AURA',
  'Финальная формула AURA',
  'Продуктовая петля',
  'Путь пользователя: первые 30 дней',
  'Life Canvas должен объяснять причинность',
  ...
  'Решение по pricing',
  'Гипотезы conversion / ARPMAU',
  'Подписка на генерации',
  'Стоимость free user и break-even conversion',
  'Финансовая модель: главный dashboard',
  'Годовые сценарии: сколько может приносить',
  'Сколько может стоить проект по стадиям',
  'Модель маркетинговой воронки AURA',
  'Маркетинг в Фазе 1: что проверяем',
...
const appendixSlideTitles = [
  'Словарь метрик простым языком',
  'Как мы выводим продукт',
  '3 рабочие продуктовые концепции',
  'Ключевые экраны',
  'Unit-экономика astrology-приложений',
  'Unit-экономика mindfulness-приложений',
  'Экономика AI companion / avatar',
  'Бенчмарки стоимости генерации',
  'Стоимость пользователя / месяц',
  'Stress-test стоимости видео',
  'Базовая финансовая модель AURA',
  'Операционная экономика при росте пользователей',
  'Помесячная финмодель: M1-M6',
  'Помесячная финмодель: M7-M12',
  'Квартальная модель: выручка, расходы, runway',
  'Маркетинговые бенчмарки',
  'Матрица маркетинговых hooks'
];

const competitorImplications = {
  title: 'Что данные конкурентов значат для AURA',
  type: 'implicationCards',
  rows: [
    ['Данные рынка', 'Что видно у лидеров', 'Что это значит для AURA', 'Решение'],
    ['Подписка работает', 'астро, mindfulness и AI companion продают $9.99-$14.99+', 'можно тестировать подписку, но не обещать безлимитный AI', 'Plus + Visual Plus + annual'],
    ['Конверсия не магическая', 'public proxies дают ориентир 2.5-5% paid conversion', 'экономика должна сходиться даже при умеренной конверсии', 'считать free-user и break-even'],
    ['Видео дорогое', 'Runway дешевле Veo, talking avatars дороже daily images', 'видео должно быть premium moment, не бесплатный daily asset', '4 видео/мес только в Visual Plus'],
    ['Рынок широкий', 'TAM/SAM/SOM есть, но вход начинается с узкого ICP', 'капитализация появляется не из “астро”, а из repeatable weekly ritual', 'сначала доказать loop и retention']
  ],
  note: 'Вывод: конкурентные цифры не доказывают, что AURA уже бизнес. Они показывают, какие цены, конверсии и ограничения нужно проверить первыми.'
};

const appendixDivider = {
  title: 'Appendix: доказательная база',
  type: 'sectionDivider',
  subtitle: 'Подробные таблицы, расчеты и backup-слайды для проверки цифр.',
  bullets: [
    'Unit-экономика конкурентов по категориям.',
    'Стоимость генерации и stress-test видео.',
    'Помесячная / квартальная финмодель.',
    'Маркетинговые benchmark и hooks.'
  ]
};

const mainSlides = mainSlideTitles.map(title => byTitle.get(title)).filter(Boolean);
const appendixSlides = appendixSlideTitles.map(title => byTitle.get(title)).filter(Boolean);
const slides = [
  ...mainSlides.slice(0, 18),
...
function writePlan() {
  const lines = ['# AURA Trendvi-Style Product Deck V8', '', 'Deck mode: optimized client-facing AURA product deck. V8 applies a premium visual pass to the client story: dense market, finance, GTM and decision slides are converted into visual frameworks while the full appendix keeps detailed proof tables.', ''];
  lines.push('## Main / Appendix split');
  lines.push('');
  lines.push(`- Main story: ${mainSlides.length + 1} slides including the competitor implications bridge.`);
  lines.push(`- Appendix: ${appendixSlides.length} backup slides after the appendix divider.`);
  lines.push('- Main flow keeps decisions, money logic, launch gates and client-readable competitor meaning.');
  lines.push('- Appendix keeps dense proof tables, detailed cost benchmarks, monthly models and marketing hooks.');
  lines.push('');
...
`;
  ...
  if (slide.type === 'sectionDivider') {
    return `
  ctx.addShape(slide, { left: 0, top: 0, width: ctx.W, height: ctx.H, fill: C.purpleLight, line: { style: 'solid', fill: C.purpleLight, width: 0 } });
  ctx.addShape(slide, { left: 74, top: 92, width: 108, height: 108, fill: C.purple, line: { style: 'solid', fill: C.purple, width: 0 } });
  ctx.addText(slide, { text: 'A', left: 94, top: 111, width: 68, height: 68, fontSize: 58, bold: true, fontFace: 'Montserrat', color: C.white, align: 'center' });
  ctx.addText(slide, { text: '${esc(slide.title)}', left: 74, top: 235, width: 930, height: 74, fontSize: 54, bold: true, fontFace: 'Montserrat', color: C.purple, fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(slide.subtitle)}', left: 78, top: 326, width: 820, height: 42, fontSize: 25, fontFace: 'Arial', color: C.ink, fit: 'shrink' });
  ${slide.bullets.map((b, i) => `
  ctx.addShape(slide, { geometry: 'ellipse', left: 92, top: ${420 + i * 44}, width: 18, height: 18, fill: ${i === 1 ? 'C.green' : 'C.purple'}, line: { style: 'solid', fill: ${i === 1 ? 'C.green' : 'C.purple'}, width: 0 } });
  ctx.addText(slide, { text: '${esc(b)}', left: 130, top: ${412 + i * 44}, width: 790, height: 28, fontSize: 23, bold: true, fontFace: 'Arial', color: C.ink, fit: 'shrink' });`).join('\n')}
`;
  }
...
`;
  ...
  if (slide.type === 'marketPoolsVisual') {
    return `
  title(slide, ctx, '${esc(slide.title)}');
  ctx.addText(slide, { text: 'AURA стоит не в одном рынке, а на пересечении четырех уже платных поведений.', left: 92, top: 112, width: 980, height: 34, fontSize: 22, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
  ${slide.rows.slice(1).map((r, i) => {
    const x = 70 + (i % 2) * 545;
    const y = 178 + Math.floor(i / 2) * 185;
    const fill = i === 3 ? 'C.greenLight' : 'C.purpleLight';
    return `
  ctx.addShape(slide, { left: ${x}, top: ${y}, width: 500, height: 145, fill: ${fill}, line: { style: 'solid', fill: C.line, width: 1.1 } });
  ctx.addText(slide, { text: '${esc(r[0])}', left: ${x + 20}, top: ${y + 16}, width: 215, height: 24, fontSize: 21, bold: true, fontFace: 'Arial', color: C.purpleDark, fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[1])}', left: ${x + 258}, top: ${y + 16}, width: 215, height: 28, fontSize: 16, bold: true, fontFace: 'Arial', color: C.ink, align: 'right', fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[2])}', left: ${x + 20}, top: ${y + 54}, width: 220, height: 54, fontSize: 16, fontFace: 'Arial', color: C.ink, fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[3])}', left: ${x + 258}, top: ${y + 54}, width: 215, height: 60, fontSize: 16, bold: true, fontFace: 'Arial', color: C.ink, fit: 'shrink' });`;
  }).join('\n')}
  ctx.addShape(slide, { left: 125, top: 610, width: 930, height: 38, fill: C.white, line: { style: 'solid', fill: C.purple, width: 1.2 } });
  ctx.addText(slide, { text: '${esc(slide.note)}', left: 145, top: 618, width: 890, height: 20, fontSize: 16, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
`;
  }

  if (slide.type === 'marketLadderVisual') {
    return `
  title(slide, ctx, '${esc(slide.title)}');
  ctx.addText(slide, { text: 'Читаем снизу вверх: beachhead — это первый масштаб проверки, venture upside — отдельный сценарий после доказанных метрик.', left: 92, top: 112, width: 990, height: 32, fontSize: 20, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
  ${slide.rows.slice(1).map((r, i) => {
    const y = 168 + i * 76;
    const fills = ['C.grey', 'C.purpleLight', 'C.greenLight', 'C.purpleLight', 'C.white'];
    return `
  ctx.addShape(slide, { left: ${86 + i * 22}, top: ${y}, width: ${1000 - i * 44}, height: 58, fill: ${fills[i]}, line: { style: 'solid', fill: C.line, width: 1 } });
  ctx.addText(slide, { text: '${esc(r[0])}', left: ${105 + i * 22}, top: ${y + 13}, width: 150, height: 24, fontSize: 20, bold: true, fontFace: 'Montserrat', color: C.purpleDark, fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[2])}', left: ${270 + i * 22}, top: ${y + 9}, width: 300, height: 34, fontSize: 15.5, bold: true, fontFace: 'Arial', color: C.ink, fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[3])}', left: ${590 + i * 22}, top: ${y + 9}, width: ${465 - i * 44}, height: 34, fontSize: 15.5, fontFace: 'Arial', color: C.ink, fit: 'shrink' });`;
  }).join('\n')}
  ctx.addText(slide, { text: '${esc(slide.note)}', left: 150, top: 595, width: 860, height: 42, fontSize: 18, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
`;
  }

  if (slide.type === 'implicationCards') {
    return `
  title(slide, ctx, '${esc(slide.title)}');
  ${metricBand(slide)}
  ${slide.rows.slice(1).map((r, i) => {
    const x = 74 + (i % 2) * 545;
    const y = 158 + Math.floor(i / 2) * 188;
    return `
  ctx.addShape(slide, { left: ${x}, top: ${y}, width: 495, height: 150, fill: ${i === 3 ? 'C.greenLight' : 'C.purpleLight'}, line: { style: 'solid', fill: C.line, width: 1.1 } });
  ctx.addText(slide, { text: '${esc(r[0])}', left: ${x + 22}, top: ${y + 16}, width: 210, height: 26, fontSize: 22, bold: true, fontFace: 'Arial', color: C.purpleDark, fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[1])}', left: ${x + 250}, top: ${y + 18}, width: 215, height: 32, fontSize: 15.5, fontFace: 'Arial', color: C.ink, align: 'right', fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[2])}', left: ${x + 22}, top: ${y + 66}, width: 235, height: 48, fontSize: 16, fontFace: 'Arial', color: C.ink, fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[3])}', left: ${x + 280}, top: ${y + 70}, width: 185, height: 38, fontSize: 18, bold: true, fontFace: 'Arial', color: C.ink, fit: 'shrink' });`;
  }).join('\n')}
  ctx.addText(slide, { text: '${esc(slide.note)}', left: 138, top: 604, width: 900, height: 38, fontSize: 17, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
`;
  }

  if (slide.type === 'phaseBudgetVisual') {
    return `
  title(slide, ctx, '${esc(slide.title)}');
  ${slide.rows.slice(1).map((r, i) => {
    const x = 64 + i * 270;
    const fill = i === 0 ? 'C.greenLight' : 'C.purpleLight';
    return `
  ctx.addShape(slide, { left: ${x}, top: 170, width: 240, height: 360, fill: ${fill}, line: { style: 'solid', fill: C.line, width: 1.1 } });
  ctx.addText(slide, { text: '${esc(r[0])}', left: ${x + 18}, top: 196, width: 205, height: 42, fontSize: 21, bold: true, fontFace: 'Arial', color: C.purpleDark, align: 'center', fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[1])}', left: ${x + 18}, top: 260, width: 205, height: 42, fontSize: 29, bold: true, fontFace: 'Montserrat', color: C.ink, align: 'center', fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[2])}', left: ${x + 22}, top: 335, width: 196, height: 78, fontSize: 15.5, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
  ctx.addShape(slide, { left: ${x + 30}, top: 445, width: 180, height: 2, fill: C.purple, line: { style: 'solid', fill: C.purple, width: 0 } });
  ctx.addText(slide, { text: '${esc(r[3])}', left: ${x + 18}, top: 468, width: 205, height: 42, fontSize: 15.5, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });`;
  }).join('\n')}
  ctx.addText(slide, { text: 'Смысл: Фаза 1 покупает не разработку всего продукта, а право принять уверенное решение о MVP.', left: 140, top: 594, width: 880, height: 36, fontSize: 20, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
`;
  }

  if (slide.type === 'phaseOneVisual') {
    return `
  title(slide, ctx, '${esc(slide.title)}');
  ${metricBand(slide)}
  ${slide.rows.slice(1).map((r, i) => {
    const x = 70 + i * 213;
    return `
  ctx.addShape(slide, { left: ${x}, top: 178, width: 178, height: 300, fill: ${i === 0 ? 'C.greenLight' : 'C.purpleLight'}, line: { style: 'solid', fill: C.line, width: 1.1 } });
  ctx.addText(slide, { text: '${esc(r[0])}', left: ${x + 12}, top: 200, width: 154, height: 40, fontSize: 19, bold: true, fontFace: 'Arial', color: C.purpleDark, align: 'center', fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[1])}', left: ${x + 12}, top: 262, width: 154, height: 34, fontSize: 22, bold: true, fontFace: 'Montserrat', color: C.ink, align: 'center', fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[2])}', left: ${x + 14}, top: 318, width: 150, height: 62, fontSize: 14.2, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[3])}', left: ${x + 14}, top: 408, width: 150, height: 45, fontSize: 14.2, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });`;
  }).join('\n')}
  ctx.addText(slide, { text: 'Фаза 1 должна дать ответы про спрос, понятность Canvas, цену и стоимость привлечения.', left: 150, top: 590, width: 860, height: 34, fontSize: 20, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
`;
  }

  if (slide.type === 'scopeVisual') {
    return `
  title(slide, ctx, '${esc(slide.title)}');
  ctx.addText(slide, { text: 'MVP должен защищать петлю. Все, что не помогает проверить причинность, оплату и возврат, уходит позже.', left: 110, top: 112, width: 940, height: 34, fontSize: 20, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
  ctx.addShape(slide, { left: 88, top: 164, width: 470, height: 390, fill: C.greenLight, line: { style: 'solid', fill: C.line, width: 1.1 } });
  ctx.addShape(slide, { left: 620, top: 164, width: 470, height: 390, fill: C.purpleLight, line: { style: 'solid', fill: C.line, width: 1.1 } });
  ctx.addText(slide, { text: 'MVP', left: 108, top: 190, width: 430, height: 34, fontSize: 30, bold: true, fontFace: 'Montserrat', color: C.ink, align: 'center' });
  ctx.addText(slide, { text: 'ПОЗЖЕ', left: 640, top: 190, width: 430, height: 34, fontSize: 30, bold: true, fontFace: 'Montserrat', color: C.ink, align: 'center' });
  ${slide.rows.slice(1).map((r, i) => `
  ctx.addText(slide, { text: '${esc(r[0])}', left: 110, top: ${250 + i * 53}, width: 120, height: 22, fontSize: 16, bold: true, fontFace: 'Arial', color: C.purpleDark, fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[1])}', left: 245, top: ${248 + i * 53}, width: 285, height: 28, fontSize: 15, fontFace: 'Arial', color: C.ink, fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[2])}', left: 650, top: ${248 + i * 53}, width: 380, height: 28, fontSize: 15, fontFace: 'Arial', color: C.ink, fit: 'shrink' });`).join('\n')}
  ctx.addText(slide, { text: 'Правило: сначала причинность Canvas и оплата, затем видео, голоса, marketplace и community.', left: 155, top: 598, width: 870, height: 30, fontSize: 19, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
`;
  }

  if (slide.type === 'riskVisual') {
    return `
  title(slide, ctx, '${esc(slide.title)}');
  ${metricBand(slide)}
  ${slide.rows.slice(1).map((r, i) => {
    const x = 70 + (i % 3) * 360;
    const y = 165 + Math.floor(i / 3) * 205;
    return `
  ctx.addShape(slide, { left: ${x}, top: ${y}, width: 310, height: 160, fill: ${i === 2 ? 'C.greenLight' : 'C.purpleLight'}, line: { style: 'solid', fill: C.line, width: 1.1 } });
  ctx.addText(slide, { text: '${esc(r[0])}', left: ${x + 18}, top: ${y + 18}, width: 274, height: 26, fontSize: 21, bold: true, fontFace: 'Arial', color: C.purpleDark, fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[1])}', left: ${x + 18}, top: ${y + 62}, width: 274, height: 32, fontSize: 15.5, fontFace: 'Arial', color: C.ink, fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[2])}', left: ${x + 18}, top: ${y + 108}, width: 274, height: 32, fontSize: 15.5, bold: true, fontFace: 'Arial', color: C.ink, fit: 'shrink' });`;
  }).join('\n')}
  ctx.addText(slide, { text: 'Главный риск не рынок, а причинность: пользователь должен понимать, почему Life Canvas изменился.', left: 150, top: 604, width: 860, height: 32, fontSize: 19, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
`;
  }

  if (slide.type === 'readinessVisual') {
    return `
  title(slide, ctx, '${esc(slide.title)}');
  ctx.addShape(slide, { left: 88, top: 104, width: 990, height: 74, fill: C.purpleLight, line: { style: 'solid', fill: C.line, width: 1.1 } });
  ctx.addText(slide, { text: 'Вердикт: запускаем проверку, но не считаем продукт доказанным бизнесом до первой группы пользователей.', left: 120, top: 120, width: 925, height: 38, fontSize: 19, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
  ${slide.rows.slice(1).map((r, i) => {
    const x = 78 + i * 213;
    return `
  ctx.addShape(slide, { left: ${x}, top: 218, width: 180, height: 300, fill: ${i === 4 ? 'C.greenLight' : 'C.white'}, line: { style: 'solid', fill: C.line, width: 1.1 } });
  ctx.addText(slide, { text: '${esc(r[0])}', left: ${x + 12}, top: 238, width: 156, height: 38, fontSize: 17.5, bold: true, fontFace: 'Arial', color: C.purpleDark, align: 'center', fit: 'shrink' });
  ctx.addText(slide, { text: '${esc(r[1])}', left: ${x + 14}, top: 300, width: 152, height: 66, fontSize: 12.8, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
  ctx.addShape(slide, { left: ${x + 28}, top: 386, width: 124, height: 1.4, fill: C.purpleLight, line: { style: 'solid', fill: C.purpleLight, width: 0 } });
  ctx.addText(slide, { text: '${esc(r[3])}', left: ${x + 14}, top: 412, width: 152, height: 70, fontSize: 12.8, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });`;
  }).join('\n')}
  ctx.addShape(slide, { left: 145, top: 592, width: 890, height: 46, fill: C.greenLight, line: { style: 'solid', fill: C.line, width: 1 } });
  ctx.addText(slide, { text: '${esc(slide.note)}', left: 170, top: 602, width: 840, height: 24, fontSize: 13.8, bold: true, fontFace: 'Arial', color: C.ink, align: 'center', fit: 'shrink' });
`;
  }
