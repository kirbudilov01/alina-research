...
lines.push('');
lines.push('Чтобы идея стала понятной для презентации, ее можно разложить на несколько продуктовых концепций. Во всех вариантах стартовый вход может быть очень простым: дата рождения, имя, время или место рождения, текущий запрос дня, настроение и короткий личный контекст. Важно, что дата рождения здесь не должна продаваться как жесткое предсказание. Она работает как символический и персональный вход: человек чувствует “это про меня”, но ценность появляется только тогда, когда этот вход превращается в действие, reset и видимый avatar/progress.');
lines.push('');
lines.push(mdTable([
  {
    concept: '1. Daily Aura Code',
    ...
    loop: 'Смысл дня превращается в действие; после выполнения avatar получает маленький визуальный слой.',
    why: 'Подходит как самый простой первый экран: быстро, лично, без тяжелого onboarding.'
  },
  {
    concept: '2. Future Self Avatar',
    input: 'Дата рождения + цель пользователя: спокойствие, уверенность, фокус, отношения, тело, деньги.',
    output: 'Образ будущей версии себя и карта качеств, которые человек постепенно “прокачивает”.',
    loop: 'Каждое действие усиливает конкретное качество avatar: ясность, смелость, мягкость, дисциплина.',
    why: 'Делает avatar центральной механикой, а не украшением профиля.'
  },
  {
    concept: '3. Ritual of the Day',
    ...
    loop: 'Reset снижает сопротивление, действие завершает ритуал, прогресс фиксируется без наказания за пропуск.',
    why: 'Связывает spiritual/wellness вход с практической ежедневной привычкой.'
  },
  {
    concept: '4. Symbol to Action',
    ...
    loop: 'Пользователь выбирает действие, делает его и видит, как символ дня становится частью avatar/progress.',
    why: 'Использует интерес к astrology/esoterics, но выводит его из чтения в поведение.'
  },
  {
    concept: '5. Inner Weather',
  ...
  {
    concept: '6. Compatibility With Myself',
    ...
    loop: 'После действия avatar показывает не “уровень”, а согласованность: человек чуть ближе к себе.',
    why: 'Хорошо раскрывает эмоциональную глубину и может стать платной персональной механикой.'
  },
  {
    concept: '7. Life Path Quests',
    input: 'Дата рождения + выбранное направление: отношения, карьера, тело, дом, творчество, уверенность.',
    output: 'Мягкая серия микро-квестов на неделю, где каждый день связан с одной темой и одним действием.',
    loop: 'Неделя собирается в историю: avatar меняется не от streak, а от накопленных доказательств действий.',
    why: 'Дает retention без давления: человек возвращается ради истории изменения, а не ради наказания.'
  }
], [
