...
lines.push('## Карта проверки гипотез');
lines.push('');
lines.push('AURA строится не как набор утверждений, а как последовательность проверок. Каждая гипотеза снимает отдельный риск: форма продукта, деньги, белое пятно, конкурентное преимущество, аудитория и продуктовый ядро.');
lines.push('');
...
lines.push('');
lines.push(pageBreak());
lines.push('## Категории как слои продукта');
lines.push('');
lines.push('Рынки важны не сами по себе. Каждый рынок приносит в AURA конкретный слой: reset, meaning, action, visual self или return. Так цифры превращаются в продуктовую архитектуру.');
lines.push('');
...
lines.push('');
lines.push(pageBreak());
lines.push('## Карта первых аудиторий');
lines.push('');
lines.push('Аудитория AURA не одна. На старте важно не охватить всех, а выбрать сегменты, которые уже живут рядом с петлей смысла, действия и визуального прогресса.');
lines.push('');
...
lines.push('');
lines.push(pageBreak());
lines.push('## Накопление ценности');
lines.push('');
lines.push('AURA должна быть понятна в первый день, но становиться ценнее на второй, седьмой и тридцатый. Если продукт не накапливает память, он остается одноразовой AI-выдачей.');
lines.push('');
lines.push('[[DIAGRAM:timeline_30]]');
lines.push('');
lines.push(pageBreak());
lines.push('## Life Canvas как причинная система');
lines.push('');
lines.push('Центральный visual moment должен показывать не новую картинку, а след действия. Поэтому Life Canvas строится как цепочка: действие -> evidence -> visual trait -> видимый образ.');
lines.push('');
lines.push('[[DIAGRAM:life_canvas_cause]]');
lines.push('');
lines.push(pageBreak());
lines.push('## Service blueprint первого loop');
lines.push('');
lines.push('Один пользовательский момент всегда имеет системную сторону: экран, backend, AI/image слой и analytics. Эта схема показывает, почему продукт нельзя проектировать только как UI.');
lines.push('');
...
lines.push('');
lines.push(pageBreak());
lines.push('## Поток данных');
lines.push('');
lines.push('Данные AURA должны идти по той же логике, что и пользовательский опыт: профиль создает контекст, season держит траекторию, action и reflection создают evidence, avatar state превращает evidence в образ.');
lines.push('');
...
lines.push('');
lines.push('Экономика AURA должна быть видна на одном экране: что стоит дешево, что становится дорогим, где появляется риск видео и почему cost per completed loop важнее красивого demo.');
lines.push('');
lines.push('[[DIAGRAM:cost_stack]]');
lines.push('');
lines.push(pageBreak());
lines.push('## Лестница монетизации');
lines.push('');
lines.push('Платность не должна появляться до первого value moment. Сначала человек проходит loop, затем получает причину платить за season, memory, styles и редкие premium moments.');
lines.push('');
...
lines.push('');
lines.push(pageBreak());
lines.push('## Карта каналов');
lines.push('');
lines.push('Каналы не равны друг другу. TikTok дает discovery, creators дают доверие, Reddit дает критику, referral проверяет shareability. Каждый канал должен отвечать на свой вопрос.');
lines.push('');
lines.push('[[DIAGRAM:channel_map]]');
lines.push('');
lines.push(pageBreak());
lines.push('## Контент-пиллары');
lines.push('');
lines.push('Контент AURA должен повторять несколько сильных углов, а не каждый день объяснять продукт заново. Это делает запуск последовательным и узнаваемым.');
lines.push('');
lines.push('[[DIAGRAM:content_wheel]]');
lines.push('');
lines.push(pageBreak());
lines.push('## Таймлайн первых 30 дней');
lines.push('');
lines.push('Запуск начинается с интервью и concierge, а не с масштабного трафика. Этот таймлайн показывает, как learning превращается в первые paid signals.');
lines.push('');
lines.push('[[DIAGRAM:launch_timeline]]');
lines.push('');
lines.push(pageBreak());
lines.push('## Доска экспериментов');
lines.push('');
lines.push('Каждый GTM-тест должен быть привязан к гипотезе и метрике. Иначе команда получит просмотры, но не поймет, что делать с продуктом.');
lines.push('');
...
lines.push('');
lines.push(pageBreak());
lines.push('## Карта зависимостей');
lines.push('');
lines.push('Команда не может строить все параллельно. Wireframes блокируют UI, prompt блокирует episode, image provider блокирует Life Canvas, RevenueCat блокирует paywall, analytics блокирует решение.');
lines.push('');
lines.push('[[DIAGRAM:dependency_map]]');
lines.push('');
lines.push(pageBreak());
lines.push('## Бюджет по спринтам');
lines.push('');
lines.push('Бюджет должен быть привязан к доказательству петли. Самые дорогие спринты оправданы только если приближают продукт к проверке causality и return.');
lines.push('');
...
lines.push('');
lines.push(pageBreak());
lines.push('## Dashboard решения');
lines.push('');
lines.push('Решение о будущем AURA должно приниматься по нескольким метрикам сразу: activation, completed loop, causality, D1 и paid intent. Одна сильная метрика не спасает слабую петлю.');
lines.push('');
lines.push('[[DIAGRAM:go_no_go]]');
lines.push('');
lines.push(pageBreak());
lines.push('## Дерево решения');
lines.push('');
lines.push('После первых пользователей команда должна выбрать: менять flow, усиливать retention или строить дальше. Дерево решения защищает от бесконечной интерпретации данных.');
lines.push('');
