...
def group_by(rows: list[dict[str, str]], key: str) -> dict[str, list[dict[str, str]]]:
    ...
    for row in rows:
        grouped.setdefault(clean(row.get(key)) or "unknown", []).append(row)
    return grouped
...
def main() -> None:
    ...
    cross_source_saturation = read_csv("data_processed/cross_source_market_saturation_matrix.csv")
    ...
    metrics = {
        ...
        "Saturation markets": number(len(cross_source_saturation)),
    ...
    story = [
        ...
        Spacer(1, 0.14 * inch),
        para("Validation Evidence Workspace", "H2"),
        para(
            "The workspace is the local intake structure for observed evidence. It keeps screenshots and notes linked to command_id before verdicts are changed.",
            "Body",
        ),
        table(
            [["Lane", "Commands", "Workspace"]]
            + [
                [
                    row.get("lane"),
                    row.get("command_rows"),
                    short(row.get("workspace_dir"), 86),
                ]
        ...
        Spacer(1, 0.14 * inch),
        para("Validation Batch 02", "H2"),
        para(
            "Batch 02 pre-creates the full non-blocker P0 workbench. Rows remain intake notes, not observed validation evidence.",
            "Body",
        ),
        table(
            [["Lane", "Rows", "Example note file"]]
            + [
                [
                    lane,
                    len(rows),
                    short(rows[0].get("note_path"), 92) if rows else "",
                ]
                for lane, rows in sorted(
                    group_by(validation_batch02, "lane").items(),
